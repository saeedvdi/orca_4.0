...
STATUS, 2026-08-16
==================
This script audits the PRE-AUDIT production decks (the four in DECK_FILES) and is
deliberately left pointing at them, so the findings below stay reproducible. The
fixes live in the 89-series decks, derived in
    scripts/refit_joint_constants_from_paper.py
and generated by
    scripts/build_paper_corrected_decks.py
Run this script to see the problem; run those two to see the answer.
...
def section_rock():
    ...
    print("      -> CORRECTED 2026-08-16: this is consumed in exactly one place, the")
    print("         `rockHM` OrcaTHMaterial on the matrix blocks, where it enters")
    print("         1/M = (alpha-phi)/K_s + phi/K_f. The fracture flow reads")
    print("         fracture_transmissivity and never sees it, so the earlier claim that")
    print("         'the same value is handed to the fracture fluid' was wrong. At")
    print("         phi = 1e-3 the phi/K_f term is ~5-12 % of 1/M, so the error moves")
    print("         matrix storage by ~6 % and nothing else. Fixed to 2.2e9 in the")
    print("         89-series decks.")
...
def section_summary():
    ...
    items = [
        ...
        ("all", "fluid_bulk_modulus = 4.78 GPa against 2.2 GPa for water",
         "~6 % on matrix storage; consumed nowhere else. Fixed in the 89-series decks"),
