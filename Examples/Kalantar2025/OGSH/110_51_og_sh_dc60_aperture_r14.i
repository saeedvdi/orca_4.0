##############################################################################
# 110_51_og_sh_dc60_aperture_r14
# GENERATED BY scripts/build_110_kalantar_round14_decks.py  -- do not hand-edit; regenerate instead.
# Parent: Examples/Kalantar2025/OGSH/110_38_og_sh_control_r13.i
#
#   characteristic_slip_distance = 6.00e-05
#       lower bracket under the NEW gouge law
#   bb_max_aperture_closure = 8.0e-06
#       V_m 1.2 -> 8.0 um. sigma_0 = V_m*K_ni was 15 MPa, so g sat at 0.96-0.99 over OG-SH's 33-43 MPa range and the closure term returned 0.04 um where Table 2 needs 0.61. NOT a fitted value -- RMS is flat above V_m = 6 um; this is the smallest sane choice within 8 % of the best
#   bb_initial_normal_stiffness = 3.00e+12
#       K_ni 1.25e13 -> 3.00e12, which with V_m = 8 um puts sigma_0 at 24.0 MPa -- inside the working range instead of below it. Fitted on the unloading branch, where slip is frozen at +3 um so a_h change is pure closure. RMS 0.084 um
#   slip_damage_scale = 2.85e-06
#       1.15 -> 2.85 um. 1.15 was Table 2's END-TO-END loss 4.87 -> 3.72, assigned entirely to gouge because closure was contributing nothing. With closure restored, sigma'_n FALLS 9.64 MPa on the loading branch and would open the joint 0.98 um, so gouge must supply 1.52 um by stage 5
#   slip_damage_characteristic_slip = 5.25e-05
#       15 -> 52.5 um, fitted to the loading-branch gouge residual. RMS 0.053 um. At 15 um the gouge saturated before stage 4, which is why every round-13 arm with more slip scored WORSE on Q
#
# Brackets D_c under the restored aperture law. Gouge now saturates over 52.5 um instead of 15, so the aperture absorbs slip differently and the optimum D_c need not be the one interpolated from round 13's runs.
##############################################################################
##############################################################################
# 110_38_og_sh_control_r13
# GENERATED BY scripts/build_110_kalantar_round13_decks.py  -- do not hand-edit; regenerate instead.
# Parent: Examples/Kalantar2025/OGSH/110_13_og_sh_bbfast_r6.i
#
#   NO PARAMETER CHANGE -- reproducibility control
#
# Byte-for-byte 110_13 except the output names. Protects every comparison below against the 0.1 pp cross-machine reproducibility floor. If this does not reproduce 110_13's tau 29 % / Q 9.3 %, STOP -- nothing else in the round can be read.
##############################################################################
# =============================================================================
...
bb_max_aperture_closure = 8.0e-06   # ROUND 14: V_m 1.2 -> 8.0 um. sigma_0 = V_m*K_ni was 15 MPa, so g sat at 0.96-0.99 over OG-SH's 33-43 MPa range and the closure term returned 0.04 um where Table 2 needs 0.61. NOT a fitted value -- RMS is flat above V_m = 6 um; this is the smallest sane choice within 8 % of the best
bb_initial_normal_stiffness = 3.00e+12   # ROUND 14: K_ni 1.25e13 -> 3.00e12, which with V_m = 8 um puts sigma_0 at 24.0 MPa -- inside the working range instead of below it. Fitted on the unloading branch, where slip is frozen at +3 um so a_h change is pure closure. RMS 0.084 um
...
slip_damage_scale = 2.85e-06   # ROUND 14: 1.15 -> 2.85 um. 1.15 was Table 2's END-TO-END loss 4.87 -> 3.72, assigned entirely to gouge because closure was contributing nothing. With closure restored, sigma'_n FALLS 9.64 MPa on the loading branch and would open the joint 0.98 um, so gouge must supply 1.52 um by stage 5

slip_damage_onset_slip = 0.0   # KALANTAR: Table 2 loses aperture from stage 1->2, so gouge accrues from first slip

slip_damage_characteristic_slip = 5.25e-05   # ROUND 14: 15 -> 52.5 um, fitted to the loading-branch gouge residual. RMS 0.053 um. At 15 um the gouge saturated before stage 4, which is why every round-13 arm with more slip scored WORSE on Q
...
exodus_file_base = results_exodus_hpc/110_51_og_sh_dc60_aperture_r14_hpc   # ROUND 14: self-named output
csv_file_base = results_csv_hpc/110_51_og_sh_dc60_aperture_r14_hpc   # ROUND 14: self-named output
checkpoint_file_base = results_checkpoint_hpc/110_51_og_sh_dc60_aperture_r14_hpc   # ROUND 14: self-named output
...
[Materials]
  ...
  [czm_contact]
    ...
    characteristic_slip_distance = 6.00e-05   # ROUND 14: lower bracket under the NEW gouge law
