# =============================================================================
# 110_18_og_sc_ktshape_r7
#
# KALANTAR 2025 ROUND 7 -- OG-SC STRESS-DEPENDENT SHEAR-STIFFNESS PROBE.
#
...
# This diagnostic stops at t=4900 s.
#
# PASS GATE: stage-5 plastic slip <=2 um, no stage-6 burst, burst at stage 7.
# =============================================================================

# =============================================================================
# 110_18_og_sc_ktshape_r7
#
# KALANTAR ET AL. (2025), specimen OG-SC (saw-cut fracture) -- ROUND 5.
# Built by scripts/build_110_kalantar_decks.py from
# Examples/YeGhasemmi2018/SWS3/93_05_sw3_final_resc1p40_ppfix.i.
#
...
#      where Table 2 says 9.73. This deck: 21.175 deg.
...
#      5.0 s in holds, snapped onto every injection breakpoint. 12133 forced steps
#      -> 2427. This, not the mesh, is what truncated OG-T at 36 % and OG-SC at 77 %.
#   2. FLOW GEOMETRY. paper_/mesh_flow_width_over_length are now really substituted
#      (0.625063 / 0.619048); round 2's regex needed a suffix only the SW-S3
...
#   JRC, JCS         4.23, 153 MPa (= UCS, section 2.1 / 3.2)
#   peak envelope    Table 2 dL_s bracket [21.365, 23.955] deg, midpoint
#                    -> phi_peak 25.31 deg at the stage-1 sigma'_n of
#                    36.10 MPa, so phi_r = phi_peak - JRC log10(JCS/sigma'_n)
#                    = 22.660 deg. tau_limit there = 17.07 MPa.
#                    ROUND 3: phi_r is BRACKETED FROM BOTH SIDES by Table 2's
#                    own dL_s column -- the joint must hold at the last locked
#                    stage and fail at stage 7, which gives
#                    21.365 < phi_r < 23.955 deg. Round 2 ran 19.156, BELOW the
#                    bracket, and burst 3 stages early at tau/tau_limit 1.0160.
#   residual         phi = 21.175 deg from Table 2 stage 7
#                    (tau 9.73 / sigma'_n 25.12 MPa).
#                    MEASURED at Table 2 stage 7, immediately after the burst (dL_s jumps 0.001 -> 0.020 mm)
#                    Upper bound from the last sliding stage: 21.175 deg.
#                    Round 1 carried the parent's 29.756 deg here, which on OG-SH was
#                    ABOVE phi_r and made the joint STRENGTHEN with slip.
#   dilation         1.327 deg = 0.5 JRC log10(JCS/sigma'_n), Barton's peak
#                    dilation. The parent's value was a Ye2018 fit (50.00 deg).
#   D_c              15.2 um -- HELD at round 3's value, Table 2 gives this joint ONE sliding stage, so the mu(s) path cannot resolve D_c; and its burst slip is a frame readout (slip = delta_tau/k_eff to 0.1 %), not a weakening measurement.
#                    Stick-slip cap 12.1 um = 0.7355 * dtau_friction / k_eff,
#                    k_eff = K_sys cos^2 theta sin theta / A = 0.1521 MPa/um.
#                    The naive cap (linear drop, whole tau drop charged to friction)
#                    would read 25.4 um -- see item C above. D_c < cap -> UNSTABLE, which is what the paper reports.
#   aperture         a_h0 1.03 um anchored at sigma'_n 36.10 MPa
#                    (Table 2 stage 1); bounds [0.116, 5.76] um bracket
#                    Table 2's observed [0.58, 1.92] um.
#                    slip_damage_scale 0.45 um.
...
#   W/L              0.62506 paper / 0.61905 mesh.
...
#   3 MPa steps at 0.03 MPa/s, held 600 s, up to 24 MPa
#   (7 stages), then back down to 6 MPa (6 stages). end_time 9100 s.
...
#   Target sigma_1 = 63.39 MPa from Table 2 stage 1 via tau = (sigma_1-sigma_3)
#   sin theta cos theta. Still worth a 200 s check against the stage-1 target:
#   tau = 13.16 MPa, sigma'_n = 36.10 MPa.
#
...
#   effective_normal_paper_frame uses sin^2(30.0) = 0.250000.
#   shear_stress_paper_frame uses sin cos(30.0) = 0.433013.
...
#   at all. V_m and K_ni are now fitted here.
#
...
#   diagnosis is wrong and tangential_viscosity comes off the round-5 plan.
# =============================================================================
# =============================================================================
...
#
# Parent: 93_05_sw3_final_resc1p40_ppfix.i
#
...
#      longer (0.09554 vs 0.07946 m) -> paper_flow_width_over_length_og_sc = 0.674. v9's Q_peak
...
# --- mesh / geometry ---
mesh_file = mesh/kalantar2025_og_sc_theta30_size3.e   # KALANTAR: OG-SC factor-3 mesh, verified 5b9fcc5
...
bulk_sin_theta = 0.4999999999999999   # KALANTAR: sin(30.0 deg)
bulk_cos_theta = 0.8660254037844387   # KALANTAR: cos(30.0 deg)
...
axial_pres_final   = -1.091687e-04   # KALANTAR: GATED series-spring: sigma_1/penalty + C_ax*(sigma_1-sigma_3), sigma_1 = 63.39 MPa, C_ax = 1.4265e-12 m/Pa
...
initial_hydraulic_aperture = 1.0300e-06   # KALANTAR: Table 2 stage 1 a_h = 1.03 um, anchored at the stage-1 sigma'_n below
...
reference_effective_normal_stress = 3.6098e+07   # KALANTAR: Table 2 stage 1 sigma'_n = 36.10 MPa -- the stress at which initial_hydraulic_aperture holds, so the closure term vanishes there
...
bb_max_aperture_closure = 2.6545e-06   # KALANTAR: V_m FITTED to Table 2's a_h(sigma'_n) over 6 slip-free pressurization stages, RMS 25 nm. The parent's 1.2e-6 put sigma_0 = V_m*K_ni at 15 MPa, BELOW this specimen's 33.9-36.1 MPa range, so the closure term was pinned near its ceiling and could deliver 0.051 um against a measured 0.570
...
bb_initial_normal_stiffness = 1.3698e+13   # KALANTAR: K_ni = sigma_0/V_m with sigma_0 = 36.36 MPa, i.e. INSIDE the operating range. K_ni itself barely moves -- V_m was the wrong constant, not the stiffness
...
slip_damage_scale = 4.5000e-07   # KALANTAR: Table 2's irreversible a_h loss, 0.45 um (sigma'_n FALLS over the same stages, so a reversible law has the wrong sign)
...
slip_damage_characteristic_slip = 1.00e-05   # KALANTAR: slip over which the gouge term saturates
min_hydraulic_aperture = 1.1600e-07   # KALANTAR: 0.2x Table 2's minimum (0.58 um) -- the round-1 floor was an inherited Ye2018 value that clipped the answer
...
max_hydraulic_aperture = 5.7600e-06   # KALANTAR: 3x Table 2's maximum (1.92 um)
...
paper_flow_width_over_length_og_sc = 0.625063   # KALANTAR: W/L, W = 49.98 mm, L = 79.9600 mm (paper frame). Eq (7) as printed is 10.3x out -- see audit section 8
mesh_flow_width_over_length_og_sc = 0.619048   # KALANTAR: W/L, W = 49.98 mm, L = 80.7369 mm (mesh frame). Eq (7) as printed is 10.3x out -- see audit section 8
ml_per_m3_per_min = 6.0e7

# --- output ---
exodus_file_base = results_exodus_hpc/110_18_og_sc_ktshape_r7_hpc   # KALANTAR: self-named
csv_file_base    = results_csv_hpc/110_18_og_sc_ktshape_r7_hpc   # KALANTAR: self-named
checkpoint_file_base = results_checkpoint_hpc/110_18_og_sc_ktshape_r7_hpc   # KALANTAR: self-named
...
[Mesh]
  ...
  [source_in]
    ...
    coord = '-0.020184231 0.0 0.015039887'   # L123p4: exact interface-node coordinate on the 123.40 mm mesh.
  ...
  [source_out]
    type = ExtraNodesetGenerator
    input = source_in
    coord = '0.020184231 0.0 0.084960113'   # L123p4: exact interface-node coordinate (was '0.023160 0 0.103981')
...
[Functions]
  ...
  [axial_disp_ramp]
      ...
      expression = 'if(t<2.0,${axial_pres_initial},if(t<55.0,${axial_pres_initial}+(${axial_pres_final}-${axial_pres_initial})*(t-2.0)/53.0,if(t<9100.0,${axial_pres_final},if(t<2850.0,${axial_pres_final}+4.5e-06*(t-2550.0)/300.0,${axial_pres_final}+4.5e-06))))'
    []
  ...
  [injection_pressure]  # SW-S3 digitized schedule (from v4 deck)
    type = PiecewiseLinear
    x = '0.0 100.0 700.0 800.0 1400.0 1500.0 2100.0 2200.0 2800.0 2900.0 3500.0 3600.0 4200.0 4300.0 4900.0 5000.0 5600.0 5700.0 6300.0 6400.0 7000.0 7100.0 7700.0 7800.0 8400.0 8500.0 9100.0'
    y = '3000000.0 6000000.0 6000000.0 9000000.0 9000000.0 12000000.0 12000000.0 15000000.0 15000000.0 18000000.0 18000000.0 21000000.0 21000000.0 24000000.0 24000000.0 21000000.0 21000000.0 18000000.0 18000000.0 15000000.0 15000000.0 12000000.0 12000000.0 9000000.0 9000000.0 6000000.0 6000000.0'
  []
...
[Materials]
  ...
  [czm_contact]
    ...
    jrc = 4.23   # PAPER Table 1 (measured). Was 23.35 -- 11.9x measured AND outside Barton's 0-20 scale.
    jcs = 1.53e+08   # PAPER Sec. 2.1 UCS. Was 3.0e8.
    residual_friction_angle_degrees = 22.660   # pins the envelope through Table 2's last stick stage (23.42 MPa, 14.26 MPa) at the measured JRC/JCS. Was 8.45.
    cohesion = 0   # 90_05: level-only correction, see header.
    residual_cohesion = 0.0   # 92_03: -0.25 MPa off 91_05, see header. 89_02 ran 0.0, 91_05 ran 1.65e6.
    ...
    characteristic_slip_distance = 1.522e-05
    slip_weakening_exponent = 1.4
    slip_weakening_residual_friction_angle_degrees = 21.175
    ...
    dilation_angle_peak_degrees = 1.327
    dilation_angle_residual_degrees = 1.327
...
[Postprocessors]
  ...
  [injection_pressure_pp]
    type = PointValue
    variable = pore_pressure
    point = '-0.020184231 0.0 0.015039887'  # L123p4: must track the source_in coord above
  []
  ...
  [pp_outlet_pp]
    type = PointValue
    variable = pore_pressure
    point = '0.020184231 0.0 0.084960113'  # L123p4: must track the source_out coord above
  []
  [pp_drop_pp]
    type = ParsedPostprocessor
  ...
  [flow_rate_validation_ml_min_pp]
    ...
    expression = '(${paper_flow_width_over_length_og_sc} / (12.0 * ${fluid_viscosity_ref})) * hydraulic_aperture_pp^3 * pp_drop_pp * ${ml_per_m3_per_min}'
  []
  [flow_rate_mesh_geometry_ml_min_pp]
    type = ParsedPostprocessor
    pp_names = 'hydraulic_aperture_pp pp_drop_pp'
    expression = '(${mesh_flow_width_over_length_og_sc} / (12.0 * ${fluid_viscosity_ref})) * hydraulic_aperture_pp^3 * pp_drop_pp * ${ml_per_m3_per_min}'
  []
  ...
  [effective_normal_paper_frame_mpa_pp]
    ...
    expression = '33.0 - 0.5*(injection_pressure_pp + pp_outlet_pp)*1e-6 + 0.250000000000000*differential_stress_reaction_mpa_pp'
  []
...
[Executioner]
  ...
  line_search = l2
  start_time = 0
  end_time = 4900   # ROUND 7 diagnostic horizon   # FULL SW-S3 cycle (11 stages, ~430 s each)

  [TimeStepper]
    type = IterationAdaptiveDT
    # ROUND 3: per-segment dt limit. Ramps (100 s, the load actually moves)
    # keep 1.5 s; holds (600 s, where round 2 converged at
    # nonlinear iteration 0 and a_h/Q/slip moved <0.1/0.4/2 %) get 5.0 s.
    # 12133 forced steps -> 2427. dtmin and the cutback are untouched,
    # so a stick-slip event still resolves itself down to 1e-6 s.
    time_t  = '0.0 0.5 99.5 100.5 699.5 700.5 799.5 800.5 1399.5 1400.5 1499.5 1500.5 2099.5 2100.5 2199.5 2200.5 2799.5 2800.5 2899.5 2900.5 3499.5 3500.5 3599.5 3600.5 4199.5 4200.5 4299.5 4300.5 4899.5 4900.5 4999.5 5000.5 5599.5 5600.5 5699.5 5700.5 6299.5 6300.5 6399.5 6400.5 6999.5 7000.5 7099.5 7100.5 7699.5 7700.5 7799.5 7800.5 8399.5 8400.5 8499.5 8500.5 9099.5 9100.0'
    time_dt = '1.50 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50'
