...
**Last updated:** 2026-08-25 (round-5 analysis / round 6 built). **Branch:** `orca_v8`,
commit `353faf3` + later results and decks.

> **State in one line, 2026-08-25.** Round 5 verified the frame mechanism on OG-SH:
> within-run unloading moved **84.1 -> 165.6 MPa/mm**, within 3.7% of the 172.0 target.
> The response over-stiffened (stage-5 slip 23.18 vs 44.6 um), so round 6 re-brackets
> `D_c` at 100 um. OG-SC's aperture nRMSE stayed 10 but its burst is one stage early,
> so round 6 isolates `tangential_viscosity` at 5x. OG-T's round-5 CSV is **48.6% and
> not scoreable**; round 6 is only a 60 s 26-degree tip-clearance probe. Decks
> `110_13`/`110_14 probe`/`110_15` are `Syntax OK`, with 64 ranks, 64 GB and 24 h.
> Write-up: `doc/KALANTAR2025_ROUND5_BACKANALYSIS.md`.
>
...
| **Round-4 decks, BUILT 2026-08-24** | `OGSH/110_07`, `OGT/110_08`, `OGSC/110_09` + `_hpc.sh` | **all `Syntax OK`; OG-SH and OG-SC ready to submit, OG-T HELD as a control** |
| **Round-4 results, 2026-08-25** | `*/results_csv_hpc/110_0{7,8,9}_*.csv` | **all three COMPLETE for the first time.** OG-SC 77 → **29**, OG-SH 17 → **31** (worse), OG-T 58 (control) |
| **Round-5 decks, BUILT 2026-08-25** | `OGSH/110_10`, `OGT/110_11`, `OGSC/110_12` + `_hpc.sh` | **all `Syntax OK`; frame fix only — submit all three** |
...
| **F5** | **Round-5 decks `110_10`/`110_11`/`110_12` BUILT and `Syntax OK`** — the frame fix, one change, three nulls. Submit all three | **ready to submit** |
| **F6** | **Round-6 decks `110_13`/`110_15` plus `110_14` 26-degree preload probe BUILT and `Syntax OK`** — corrected-frame D_c bracket, viscosity arm, geometry falsifier | **ready to submit, 64 ranks / 64 GB / 24 h each** |
| **F4** | **Round-4 decks `110_07`/`110_08`/`110_09` BUILT and `Syntax OK`** — four constants, two new derivations, four preregistered nulls. Submit OG-SH + OG-SC | **ready to submit** |
...
| 2026-08-24 (round 4 built) | `orca_v8` | **Round-4 decks built: `110_07` OG-SH, `110_08` OG-T, `110_09` OG-SC, all `Syntax OK`.** FOUR constants move, verified by diff — OG-SH one, OG-SC three, **OG-T none**. OG-SC `slip_weakening_residual` 15.354 -> **21.175 deg** (read after its burst, not at a locked stage); OG-SH `characteristic_slip_distance` 150 -> **59.3 um**; OG-SC `V_m` 1.20 -> **2.655 um** and `K_ni` 1.25e13 -> **1.3698e13**. Builder gained `sliding_stages()`, `burst_index()`, `residual_angle()`, `fit_characteristic_slip()`, `fit_closure()`; the stability cap is corrected to `0.7355*dtau_mu/k_eff` and **reported, not asserted**, because in its old form it blocked the right `D_c` for three rounds. **Two things the build itself found:** (i) the 26.5 um estimate in the back-analysis was wrong — it charged the whole tau drop to friction when the BB peak also falls with sigma'_n; fitting the material's own law gives 59.3 um at 0.281 MPa RMS. (ii) The new residual rule **condemned OG-SH too** — its stages 7/8/9 all print dL_s = 0.042 mm — so OG-SH's residual is a BOUND, not a measurement, and a joint (D_c, phi_res) fit is degenerate over the 45 um Table 2 covers. Four preregistered nulls written into the deck headers, one of them as an expected FAILURE. **Local smoke test abandoned: the workstation OOM-killed it at step 1** (30 GB against a 64 GB HPC job) — an environment limit, not a deck defect |
