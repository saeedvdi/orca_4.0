...
> **State in one line, 2026-08-25.** Round 4 ran complete on all three. **OG-SC's aperture null
> PASSED outright (39 → 10 nRMSE)**; OG-SH's `D_c` fit overshot and the split bracket it produced
> exposed the campaign's real defect — **the frame stiffness is counted twice, so the joint has
> been unloading against 0.59× the experiment's stiffness for four rounds.** Round-5 decks
> `110_10`/`110_11`/`110_12` are built and fix exactly that. Write-up:
> `doc/KALANTAR2025_ROUND4_BACKANALYSIS.md`.
>
