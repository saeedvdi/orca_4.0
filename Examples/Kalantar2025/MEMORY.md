...
**Last updated:** 2026-08-31 (round 11 wave A analysed; OG-T root cause FOUND). **Branch:** `orca_v11`.
Previously 2026-08-31 (round 11 built) and 2026-08-28 (final audit / round 10 built) on `orca_v10`, commit `353faf3`.

> **OG-T ROOT CAUSE FOUND, 2026-08-31, from round-11 wave A.** **The preload is undrained.**
> OG-T's fracture is 0.10 um and carries no flow — which is faithful to Kalantar's own
> Table 2 — so the specimen has no drainage path, and the 53 s preload ramp pressurises it
> like a closed vessel: interface pore pressure rises **2.2 -> 51.1 MPa**, and with
> `fault_pressure_coefficient = 1.0` that removes **46.7 of the joint's 64.5 MPa** of
> effective normal stress. Measured Skempton B = 0.914 against 0.930 predicted from the
> deck's own constants: **the model is doing correct undrained poroelasticity on the wrong
> protocol.** OG-SH is unaffected because its 4.87 um fracture is 119 000x more transmissive
> and bleeds the overpressure away as fast as the ramp makes it (dp/dsigma_d 0.017 vs OG-T's
> 0.372-0.416).
>
> **Round 11 is refuted and its null fired.** `110_30`/`110_31` fail all three gate criteria
> (ratio 0.520/0.518, slope -0.110/-0.102, yield at sigma_d 57.6/64.0); `110_32` locks the
> joint, moves 6.7 um, and *still* shows ratio 0.376. In-plane platen freedom is not the
> mechanism, and neither is any other mechanical hypothesis — the shielding is not mechanical.
> Add alpha_f*p back and the total-stress slope is +0.277 on OG-T and +0.277 on OG-SH: load
> reaches the joint correctly.
>
> **DO NOT SUBMIT round-11 wave B** (`110_33`/`110_34`/`110_35`) — all three were gated on
> `110_30` passing. Full argument, tables and gates:
> `Doc/Memory/KALANTAR2025_ROUND11_BACKANALYSIS.md`.
>
> **ROUND 12 IS BUILT AND READY, 2026-08-31.** `110_36_og_t_drained_preload_r12` (the gate,
> preload only) and `110_37_og_t_drained_full_r12` (the full 17-stage cycle on the same
> preload), both `Syntax OK`, generated by `scripts/make_110_round12_drained_preload.py` from
> `110_29_og_t_graded_full_r10.i`. Ramp 2 -> 10 000 s (10 consolidation times) then hold to
> 14 000 s; the injection schedule is shifted +13 945 s and its time-stepping segments are the
> parent's, so the calibrated window is stepped exactly as the parent stepped it. Submit with
> `sbatch submit_110_round12_hpc.sh` (64 ranks / 64 GB / 48 h, array 0-1). **Read task 0 and
> score it before touching task 1**; if the gate fails, discard task 1 rather than analysing
> it. `scripts/score_110_round11.py` now prints `dp_MPa`, the fifth gate.
...
>
> **REFUTED 2026-08-31 by its own null** — see the root-cause block at the top of this file
> and `Doc/Memory/KALANTAR2025_ROUND11_BACKANALYSIS.md`. The hypothesis above is wrong: the
> locked-joint null shows the same deficit with 6.7 um of slip, so the shielding is not
> kinematic. It is pore pressure generated by an undrained preload.
>
...
| 2026-08-31 (round 11 wave A analysed) | `orca_v11` | **Round-11 hypothesis refuted and its preregistered null fired.** `110_30`/`110_31` fail all three gate criteria; `110_32` (locked joint, 6.7 um of slip) still shows ratio 0.376, so the shielding is not kinematic and no platen BC repairs it. **Root cause found instead: the OG-T preload is undrained.** Its fracture is 0.10 um and carries zero flow — faithful to Kalantar's Table 2 — so the specimen cannot drain, and the 53 s ramp raises interface pore pressure 2.2 -> 51.1 MPa, removing 46.7 of the joint's 64.5 MPa of effective normal stress. Skempton B = 0.914 measured against 0.930 predicted from the deck's own constants; adding alpha_f*p back gives a total-stress slope of +0.277 on OG-T and +0.277 on OG-SH. Four candidate causes ruled out by data: poroelastic constants (byte-identical across specimens), ramp duration (identical), drainage ports (single node in all three), axial BC type (displacement 0.376 vs traction 0.379 on the same specimen — round 7 was testing a variable that does not matter). OG-SH is unaffected because a_h^3 is 119 000x larger. **Wave B withdrawn.** Round-12 design: one OG-T probe, preload ramp 53 s -> >=1e4 s, five preregistered gates. `Doc/Memory/KALANTAR2025_ROUND11_BACKANALYSIS.md` |
