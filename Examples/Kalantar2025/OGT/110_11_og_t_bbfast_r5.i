# =============================================================================
# 110_11_og_t_bbfast_r5
#
# KALANTAR ET AL. (2025), specimen OG-T (tensile fracture) -- ROUND 5.
...
#      where Table 2 says 9.73. This deck: 27.414 deg.
...
#      (0.586898 / 0.576170); round 2's regex needed a suffix only the SW-S3
...
#
# DERIVED CONSTANTS
#   theta            28.0 deg  (Table 1; the geometric argument, not Table 2 s 25.999)
...
#                    63.86 MPa, so phi_r = phi_peak - JRC log10(JCS/sigma'_n)
#                    = 43.135 deg. tau_limit there = 70.25 MPa.
#                    ROUND 3: UNCHANGED. This specimen's envelope cannot be
#                    judged yet -- see the WARNING block below.
#   residual         phi = 27.414 deg from Table 2 stage 17
#                    (tau 20.41 / sigma'_n 39.35 MPa).
#                    BOUNDED, not measured: this joint is still weakening when the test ends. Lowest mobilised friction is Table 2 stage 17; the upper bound is 35.337 deg from the last sliding stage 10
#                    Upper bound from the last sliding stage: 35.337 deg.
#                    Round 1 carried the parent's 29.756 deg here, which on OG-SH was
#                    ABOVE phi_r and made the joint STRENGTHEN with slip.
#   dilation         2.296 deg = 0.5 JRC log10(JCS/sigma'_n), Barton's peak
...
#                    Stick-slip cap 133.4 um = 0.7355 * dtau_friction / k_eff,
#                    k_eff = K_sys cos^2 theta sin theta / A = 0.1485 MPa/um.
#                    The naive cap (linear drop, whole tau drop charged to friction)
#                    would read 310.4 um -- see item C above. D_c < cap -> UNSTABLE, which is what the paper reports.
#   aperture         a_h0 0.10 um anchored at sigma'_n 63.86 MPa
#                    (Table 2 stage 1); bounds [0.010, 5.00] um bracket
#                    Table 2's observed [0.00, 1.11] um.
#                    slip_damage_scale 0.00 um.
#   K_sys            796 kN/mm MEASURED (section 2.3) -> penalty 9.632e+11 Pa/m.
#                    This is the constant whose x2 bracket moved Q by -94/+408 % on
#                    Ye2018 and had to be inferred there. Here it is data.
#   W/L              0.58690 paper / 0.57617 mesh.
...
#   1.4265e-12 m/Pa (0.8987 L/E, calibrated on the completed OG-SH run 19444645).
...
#   tau = 66.50 MPa, sigma'_n = 63.86 MPa.
#
...
#   effective_normal_paper_frame uses sin^2(28.0) = 0.220404.
#   shear_stress_paper_frame uses sin cos(28.0) = 0.414519.
#   The 4 bulk-gauge PointValues span a 90 mm chord about z = 0.0500 m, the
...
mesh_file = mesh/kalantar2025_og_t_theta28_size3.e   # KALANTAR: OG-T factor-3 mesh, verified 5b9fcc5
...
exodus_file_base = results_exodus_hpc/110_11_og_t_bbfast_r5_hpc   # KALANTAR: self-named
csv_file_base    = results_csv_hpc/110_11_og_t_bbfast_r5_hpc   # KALANTAR: self-named
checkpoint_file_base = results_checkpoint_hpc/110_11_og_t_bbfast_r5_hpc   # KALANTAR: self-named
...
[Mesh]
  ...
  [source_in]
    ...
    coord = '-0.020362222 0.0 0.011704230'   # 93_01: the node ExtraNodesetGenerator ACTUALLY snaps to (use_closest_node). Was '-0.019260000 0.0 0.033577557', 1.678 mm off-node.
  ...
  [source_out]
    type = ExtraNodesetGenerator
    input = source_in
    coord = '0.020362222 0.0 0.088295770'   # 93_01: ditto. Actual inj-prod separation 69.335 mm, not the intended 72.690 mm -- see banner.
