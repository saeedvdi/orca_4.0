##############################################################################
# 110_56_og_t_drained_start_t2stress_r14
...
#
#   DRAINED START: pore pressure at production_pressure, axial stress in
#   initial_stress, axial command held from t = 0. No preload ramp.
#
#   initial_stress = '-3.3e+07 -3.3e+07 -1.8549e+08'
#       axial component matched to the reduced sigma_1. The command and the initial stress MUST agree or step 1 is not an equilibrium and the probe measures a transient it was built to avoid
#   axial_pres_final = -4.101050e-04
#       sigma_1 193.43 -> 185.49 MPa, recomputed through the deck's own series spring sigma_1/penalty + C_ax*(sigma_1-sigma_3) with penalty 9.6319e11 and C_ax 1.4265e-12. 185.49 is what Table 2 stage 1 implies: sigma_d = tau/(sin.cos) = 63.21/0.4145 = 152.49 at 28 deg
#
# Separates 'the deck is overloaded by 7.94 MPa' from 'Kalantar's reduced stresses do not stand up'. Table 2 stage 1 needs tau/sigma'_n = 1.0654, i.e. a 46.81 deg friction angle. If BOTH drained arms still yield, that is a finding about the data, and it lands next to the reading notes' two existing doubts -- the 26 deg angle the geometry cannot realise, and phi_r = 43.1 deg.
##############################################################################
# =============================================================================
...
axial_pres_final = -4.101050e-04   # ROUND 14: sigma_1 193.43 -> 185.49 MPa, recomputed through the deck's own series spring sigma_1/penalty + C_ax*(sigma_1-sigma_3) with penalty 9.6319e11 and C_ax 1.4265e-12. 185.49 is what Table 2 stage 1 implies: sigma_d = tau/(sin.cos) = 63.21/0.4145 = 152.49 at 28 deg

youngs_modulus = 6.3e+10   # KALANTAR: section 2.1
poissons_ratio = 0.16   # KALANTAR: section 2.1
strain_model = incremental
initial_stress = '-3.3e+07 -3.3e+07 -1.8549e+08'   # ROUND 14: axial component matched to the reduced sigma_1. The command and the initial stress MUST agree or step 1 is not an equilibrium and the probe measures a transient it was built to avoid
...
exodus_file_base = results_exodus_hpc/110_56_og_t_drained_start_t2stress_r14_hpc   # ROUND 14: self-named output
csv_file_base = results_csv_hpc/110_56_og_t_drained_start_t2stress_r14_hpc   # ROUND 14: self-named output
checkpoint_file_base = results_checkpoint_hpc/110_56_og_t_drained_start_t2stress_r14_hpc   # ROUND 14: self-named output
...
[ICs]
  [pp_ic]
    ...
    value = ${production_pressure}   # ROUND 14: drained start -- the specimen begins in pore-pressure equilibrium at the production value, not 2 MPa above it
  []
[]
...
[Functions]
  ...
  [axial_disp_ramp]
      ...
      expression = '${axial_pres_final}'   # ROUND 14: no ramp -- the axial command is held at its preloaded value from t = 0
    []
  ...
  [injection_pressure]
    ...
    x = '0.0 600.0'   # ROUND 14: preload probe, no injection schedule
    y = '3000000.0 3000000.0'   # ROUND 14: held at production_pressure
  []

  [event_dt_cap]
    type = PiecewiseConstant
    x = '0 2.0'   # ROUND 14: no event to resolve
    y = '5.0 5.0'   # ROUND 14: see x
  []
...
[Executioner]
  ...
  line_search = l2
  start_time = 0
  end_time = 600   # ROUND 14: 600 s probe: long enough to show the state is an equilibrium and that pore pressure stays at the production value
  ...
  [TimeSteppers]
    [adaptive]
      ...
      time_t = '0.0 0.5 2.0 600.0'   # ROUND 14: drained-start probe
      time_dt = '0.10 0.10 5.00 5.00'   # ROUND 14: fine first steps to settle the initial state, then coarse
  ...
  dtmax = 5.0   # ROUND 14: segment limits govern
  dtmin = 1e-6
  l_max_its = 50
  l_tol = 1e-4
  nl_max_its = 70
