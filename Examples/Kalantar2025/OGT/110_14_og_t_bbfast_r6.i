# =============================================================================
# 110_14_og_t_bbfast_r6
#
# KALANTAR ET AL. (2025), specimen OG-T (tensile, 26-degree diagnostic arm fracture) -- ROUND 6.
# Built by scripts/build_110_kalantar_decks.py from
# Examples/YeGhasemmi2018/SWT1/93_01_swt1_final_c26p9_resc9p19_ppfix.i.
#
# ------------------------------------------------------------------------------
...
#      where Table 2 says 9.73. This deck: 27.069 deg.
...
#      5.0 s in holds, snapped onto every injection breakpoint. 9067 forced steps
#      -> 2153. This, not the mesh, is what truncated OG-T at 36 % and OG-SC at 77 %.
#   2. FLOW GEOMETRY. paper_/mesh_flow_width_over_length are now really substituted
#      (0.548019 / 0.538001); round 2's regex needed a suffix only the SW-S3
...
#
# DERIVED CONSTANTS
#   theta            26.0 deg  (Table 1; the geometric argument, not Table 2 s 25.999)
...
#   JRC, JCS         12.10, 153 MPa (= UCS, section 2.1 / 3.2)
#   peak envelope    Figure 3, mu 1.1 / c 0.0 MPa
#                    -> phi_peak 47.73 deg at the stage-1 sigma'_n of
#                    59.33 MPa, so phi_r = phi_peak - JRC log10(JCS/sigma'_n)
#                    = 42.748 deg. tau_limit there = 65.26 MPa.
#                    ROUND 3: UNCHANGED. This specimen's envelope cannot be
#                    judged yet -- see the WARNING block below.
#   residual         phi = 27.069 deg from Table 2 stage 17
#                    (tau 19.40 / sigma'_n 37.96 MPa).
#                    BOUNDED, not measured: this joint is still weakening when the test ends. Lowest mobilised friction is Table 2 stage 17; the upper bound is 35.303 deg from the last sliding stage 10
#                    Upper bound from the last sliding stage: 35.303 deg.
#                    Round 1 carried the parent's 29.756 deg here, which on OG-SH was
#                    ABOVE phi_r and made the joint STRENGTHEN with slip.
#   dilation         2.489 deg = 0.5 JRC log10(JCS/sigma'_n), Barton's peak
#                    dilation. The parent's value was a Ye2018 fit (16.44 deg).
#   D_c              150.0 um -- HELD at round 3's value, its loading is broken before injection begins (the preload inverts dsigma'_n/dsigma_1), so no fit to its response is meaningful -- held at round 3's value as a control.
#                    Stick-slip cap 132.9 um = 0.7355 * dtau_friction / k_eff,
#                    k_eff = K_sys cos^2 theta sin theta / A = 0.1437 MPa/um.
#                    The naive cap (linear drop, whole tau drop charged to friction)
#                    would read 304.9 um -- see item C above. D_c < cap -> UNSTABLE, which is what the paper reports.
#   aperture         a_h0 0.10 um anchored at sigma'_n 59.33 MPa
#                    (Table 2 stage 1); bounds [0.010, 5.00] um bracket
#                    Table 2's observed [0.00, 1.11] um.
#                    slip_damage_scale 0.00 um.
#   K_sys            796 kN/mm MEASURED (section 2.3) -> penalty 1.026e+12 Pa/m.
#                    This is the constant whose x2 bracket moved Q by -94/+408 % on
#                    Ye2018 and had to be inferred there. Here it is data.
#   W/L              0.54802 paper / 0.53800 mesh.
...
#   3 MPa steps at 0.03 MPa/s, held 300 s, up to 30 MPa
#   (9 stages), then back down to 6 MPa (8 stages). end_time 6800 s.
...
#   1.4904e-12 m/Pa (0.8987 L/E, calibrated on the completed OG-SH run 19444645).
#   Target sigma_1 = 193.43 MPa from Table 2 stage 1 via tau = (sigma_1-sigma_3)
#   sin theta cos theta. Still worth a 200 s check against the stage-1 target:
#   tau = 63.21 MPa, sigma'_n = 59.33 MPa.
#
...
#   effective_normal_paper_frame uses sin^2(26.0) = 0.192169.
#   shear_stress_paper_frame uses sin cos(26.0) = 0.394005.
#   The 4 bulk-gauge PointValues span a 90 mm chord about z = 0.0522 m, the
...
# =============================================================================
# =============================================================================
# 93-SERIES -- MESH AND POSTPROCESSOR AUDIT FIXES.  SW-T1 mesh 5
# Built from 91_02_swt1_bbfast_c26p9_resc9p19_kernel_SV_biot0p6.i.  Constitutive parameters are UNCHANGED;
...
#   1. SOURCE COORDINATE, honesty fix, NO numerical change.  The deck asked for
#      +-0.019260000 / z 0.033577557 & 0.095222443, which is 1.678 mm from the
#      nearest fracture_interface node; ExtraNodesetGenerator with
#      use_closest_node = true silently snapped it and never warned.  The seven
#      other Ye2018 decks are exact to 0.00 um.  The coordinate now names the node
#      that was always being used, so the deck is reproducible.
#      RESIDUAL LIMITATION: the mesh-5 fracture-line node pitch is 4.333 mm, so the
#      reachable symmetric separations bracketing the intended 72.690 mm are
#      69.335 and 78.002 mm.  69.335 (-4.62 %) is the closer of the two and is what
#      this deck uses -- it cannot be improved without remeshing.  The mesh-3
#      sibling reaches 71.501 mm, so the SW-T1 convergence pair carries a 3.1 %
#      source-separation difference that is NOT discretisation.  Q enters through
#      pp_drop_pp, so expect a first-order effect on Q of that size.
...
#      (32.0 deg), not copied from SW-S4.
#
...
#
# Parent: 90_01_swt1_bbfast_cohesion_c26p4_kernel_SV_biot0p6.i
#
...
#
# THIS DECK.  Same peak cohesion as 91_01 (2.688e7 Pa, derivation in 91_01's header) but only
# HALF the residual cut: residual_cohesion 1.1176e7 -> 9.19e6 Pa (-1.99 MPa).
#
# WHY BRACKET.  The -3.97 MPa in 91_01 is a linear extrapolation from a single endpoint error
# and assumes the residual state is strength-limited rather than load-train-limited.  If SW-T1's
# post-burst stress is partly set by the frame unloading, the full cut will overshoot.  This
# deck is the conservative half-step; between the two the residual is bracketed, exactly as
# 90_01/90_02 bracketed the peak.
#
# PREDICTION.  Same onset as 91_01 (the peak envelope is identical, so any onset difference
# between these two decks is a post-onset feedback and worth knowing about).  Final sigma_d
# 66-68 MPa -- i.e. still high, but half the 90_01 error.  If 91_02 lands closer than 91_01,
# the residual is not purely a cohesion deficit and the load train needs looking at.
#
# =============================================================================================
# ==============================================================================
# 90_01_swt1_bbfast_cohesion_c26p4_kernel_SV_biot0p6
# GENERATED 2026-08-16 by scripts/build_paper_corrected_decks.py from
#   SWT1/87_01_swt1_bbfast_injfix_kernel_SV_biot0p6.i
...
#
# STATUS: CANDIDATE, not a correction. It must be scored against Table 2 before it replaces 87_01. Peak strength at the calibration point is identical by construction, but d(tau)/d(sigma'_n) changes 0.928 -> 0.554, so the post-onset trajectory will differ materially.
# ==============================================================================
# ==============================================================================
# 87_01_swt1_bbfast_injfix_kernel_SV_biot0p6
# GENERATED 2026-08-16 from Ye2018_SWT1_BBFast_sweep_19_F0p95_Pp0p60_T40p00_U0p94_A0p0160_Kinematic_IOsafe_kernel_SV_biot0p6.i -- injection schedule rebuilt from the
# 2026-08-16 re-extracted validation curve. Only the injection_pressure function, the event_dt_cap
#   window (shifted -210 s to follow the earlier peak), and the output
# file_base names differ from the parent deck.
# ==============================================================================
# 87_01_swt1_bbfast_injfix_kernel_SV_biot0p6
# GENERATED 2026-08-15 from Ye2018_SWT1_BBFast_sweep_19_F0p95_Pp0p60_T40p00_U0p94_A0p0160_Kinematic_IOsafe.i -- do not hand-edit; regenerate instead.
#
...
#      in Ye2018_SWT1_BBFast_sweep_19_F0p95_Pp0p60_T40p00_U0p94_A0p0160_Kinematic_IOsafe.i.
...
#
# The parent deck Ye2018_SWT1_BBFast_sweep_19_F0p95_Pp0p60_T40p00_U0p94_A0p0160_Kinematic_IOsafe.i is left untouched as the reference configuration.
# ==============================================================================
################################################################################
# I/O-SAFE RECOVERY SWEEP V4 SWT1-19: KINEMATIC APERTURE
# Parent/control: Ye2018_SWT1_BBFast_sweep_18_F0p95_Pp0p60_T40p00_U0p94_A0p0160.
# Purpose: complete the physically unchanged SWT1-v18 setup with bounded output.
...
# post-slip tail friction angle = 40.000 deg
# normal-unload retention fraction U = 0.940000
# kinematic hydraulic aperture scale A = 0.01600000
# hydraulic normal-closure type = barton_bandis
# Robustness: smooth unilateral contact, safeguarded/recoverable local return map,
# 5 um slip and 1.5 um dilation increment bounds, and dt <= 0.05 s over the
# 1740--1825 s event window. I/O safety: CSV every step; Exodus only at FINAL;
# one checkpoint generation retained every 800 time steps.
################################################################################

################################################################################
# Ye and Ghassemi (2018) SW-T1: minimal timing recalibration v2
#
# Geometry: H=0.12880 m, D=0.05052 m,
# fracture angle=32.0 deg, mesh=mesh/ye2018_sw_T1_mesh_size_5.e
# Paper properties: E=67 GPa, nu=0.32, UCS/JCS=150 MPa, JRC=15.32
...
# Table-2 anchors:
#   basic friction angle       = 42.9071 deg (24 MPa pre-slip hold)
#   weakened friction angle    = 42.7146 deg (28 MPa post-slip hold)
#   dilation angle             = 16.4420 deg (peak |dn|/ds)
#   hydraulic dilation scale   = 0.015414
#   retained aperture fraction = 0.714876
#   Eq.-9 flow W/L             = 0.814324
#
# REQUIRED PRE-FULL-RUN GATE:
#   (1) t=55 s plastic slip must be zero;
#   (2) first 8 MPa hold should give q~149.445 MPa and
#       tau~67.16 MPa;
...
mesh_file = mesh/kalantar2025_og_t_theta26_size3.e   # KALANTAR: OG-T factor-3 mesh, verified 5b9fcc5
sample_radius = 0.02499   # KALANTAR: Table 1, D = 49.98 mm
sample_area = 1.961924926e-03   # KALANTAR: pi r^2
bulk_sin_theta = 0.4383711467890774   # KALANTAR: sin(26.0 deg)
bulk_cos_theta = 0.8987940462991670   # KALANTAR: cos(26.0 deg)
axial_bc_penalty = 1.0264e+12   # KALANTAR: MEASURED K_sys 796 kN/mm / A -- Ye2018 had to infer this

axial_pres_initial = -3.215237e-05   # KALANTAR: -sigma_3/penalty, t=0 equilibrium
axial_pres_final = -4.275780e-04   # KALANTAR: GATED series-spring: sigma_1/penalty + C_ax*(sigma_1-sigma_3), sigma_1 = 193.43 MPa, C_ax = 1.4904e-12 m/Pa
...
reference_effective_normal_stress = 5.9330e+07   # KALANTAR: Table 2 stage 1 sigma'_n = 59.33 MPa -- the stress at which initial_hydraulic_aperture holds, so the closure term vanishes there
...
paper_flow_width_over_length_og_t = 0.548019   # KALANTAR: W/L, W = 49.98 mm, L = 91.2013 mm (paper frame). Eq (7) as printed is 10.3x out -- see audit section 8
mesh_flow_width_over_length_og_t = 0.538001   # KALANTAR: W/L, W = 49.98 mm, L = 92.8995 mm (mesh frame). Eq (7) as printed is 10.3x out -- see audit section 8
ml_per_m3_per_min = 6.0e7

exodus_file_base = results_exodus_hpc/110_14_og_t_bbfast_r6_hpc   # KALANTAR: self-named
csv_file_base    = results_csv_hpc/110_14_og_t_bbfast_r6_hpc   # KALANTAR: self-named
checkpoint_file_base = results_checkpoint_hpc/110_14_og_t_bbfast_r6_hpc   # KALANTAR: self-named
...
[Mesh]
  ...
  [source_in]
    ...
    coord = '-0.020362222 0.0 0.010491258'   # 93_01: the node ExtraNodesetGenerator ACTUALLY snaps to (use_closest_node). Was '-0.019260000 0.0 0.033577557', 1.678 mm off-node.
  ...
  [source_out]
    type = ExtraNodesetGenerator
    input = source_in
    coord = '0.020362222 0.0 0.093988742'   # 93_01: ditto. Actual inj-prod separation 69.335 mm, not the intended 72.690 mm -- see banner.
...
[Functions]
  ...
  [injection_pressure]
    ...
    x = '0.0 100.0 400.0 500.0 800.0 900.0 1200.0 1300.0 1600.0 1700.0 2000.0 2100.0 2400.0 2500.0 2800.0 2900.0 3200.0 3300.0 3600.0 3700.0 4000.0 4100.0 4400.0 4500.0 4800.0 4900.0 5200.0 5300.0 5600.0 5700.0 6000.0 6100.0 6400.0 6500.0 6800.0'
    y = '3000000.0 6000000.0 6000000.0 9000000.0 9000000.0 12000000.0 12000000.0 15000000.0 15000000.0 18000000.0 18000000.0 21000000.0 21000000.0 24000000.0 24000000.0 27000000.0 27000000.0 30000000.0 30000000.0 27000000.0 27000000.0 24000000.0 24000000.0 21000000.0 21000000.0 18000000.0 18000000.0 15000000.0 15000000.0 12000000.0 12000000.0 9000000.0 9000000.0 6000000.0 6000000.0'
  []
...
[Materials]
  ...
  [czm_contact]
    ...
    jrc = 12.10   
    jcs = 1.53e+08   
    residual_friction_angle_degrees = 42.748   # granite basic friction, measured on this campaign's own saw cut (SW-S3). Was 44.1, above every measured granite value.
    ...
    slip_weakening_residual_friction_angle_degrees = 27.069   # slip destroys ROUGHNESS, not the rock's basic friction angle -- Barton's own picture. Was 40.
    ...
    dilation_angle_peak_degrees = 2.489   
    dilation_angle_residual_degrees = 2.489   
...
[Postprocessors]
  ...
  [effective_normal_paper_frame_mpa_pp]
    ...
    expression = '33.0 - 0.5*(injection_pressure_pp + pp_outlet_pp)*1e-6 + 0.192169262337171*differential_stress_reaction_mpa_pp'
  []
  [shear_stress_paper_frame_mpa_pp]
    type = ParsedPostprocessor
    pp_names = differential_stress_reaction_mpa_pp
    expression = '0.394005376803361*differential_stress_reaction_mpa_pp'
  []
...
[Executioner]
  ...
  [TimeSteppers]
    [adaptive]
      ...
      time_t  = '0.0 0.5 99.5 100.5 399.5 400.5 499.5 500.5 799.5 800.5 899.5 900.5 1199.5 1200.5 1299.5 1300.5 1599.5 1600.5 1699.5 1700.5 1999.5 2000.5 2099.5 2100.5 2399.5 2400.5 2499.5 2500.5 2799.5 2800.5 2899.5 2900.5 3199.5 3200.5 3299.5 3300.5 3599.5 3600.5 3699.5 3700.5 3999.5 4000.5 4099.5 4100.5 4399.5 4400.5 4499.5 4500.5 4799.5 4800.5 4899.5 4900.5 5199.5 5200.5 5299.5 5300.5 5599.5 5600.5 5699.5 5700.5 5999.5 6000.5 6099.5 6100.5 6399.5 6400.5 6499.5 6500.5 6799.5 6800.0'
      time_dt = '1.50 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50 1.50 5.00 5.00 1.50'
