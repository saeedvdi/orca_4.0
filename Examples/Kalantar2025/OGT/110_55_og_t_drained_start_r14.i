...
#
#
# Drained preloaded equilibrium at the deck's gated sigma_1 = 193.43 MPa. No ramp, so no undrained transient. Gate on dp <= 3 (it should be ~0 by construction -- if it is not, the initial state is not an equilibrium and the deck is wrong), then on yield and slip.
##############################################################################
# =============================================================================
...
initial_stress = '-3.3e+07 -3.3e+07 -1.9343e+08'   # ROUND 14: axial component set to the preloaded sigma_1 so the specimen STARTS loaded. Rounds 12 and 13 proved no ramp length and no drainage boundary can drain k = 1.4e-20 m^2 in the loading time; the remaining honest option is not to simulate the preload transient at all
...
exodus_file_base = results_exodus_hpc/110_55_og_t_drained_start_r14_hpc   # ROUND 14: self-named output
csv_file_base = results_csv_hpc/110_55_og_t_drained_start_r14_hpc   # ROUND 14: self-named output
checkpoint_file_base = results_checkpoint_hpc/110_55_og_t_drained_start_r14_hpc   # ROUND 14: self-named output
