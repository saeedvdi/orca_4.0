# =============================================================================
# 106_08 STRENGTH-LEVEL PROBE -- SW-S4
# Parent: 93_07_sw4_final_theta30_jrc5_ppfix.i  (mean nRMSE 6.139 %, best SW-S4
# on disk and now the WORST of the four specimens)
#
# ONE LINE CHANGES: bb_residual_friction_angle 22.72 -> 21.60 deg.
#
# THE RESIDUAL IS ONE STAGE, AND IT IS A STRENGTH LEVEL, NOT AN ONSET SWITCH.
# The parent's tau error by hold stage is
#   -0.21 -0.16 +0.25 [+2.74] +1.06 +1.26 +0.89 +0.65 +0.47 +0.29 +0.05  MPa,
# i.e. one 2.74 MPa spike at stage 4 (Pi = 20 MPa) and a decaying tail.  At that
# stage Table 2 has slipped 0.017 mm and this model has slipped 0.0034 mm.
#
# The joint is NOT stuck there.  limit_tau_pp at stage 4 is 12.291 MPa against a
# model tau of 12.121 -- it is sitting on its own yield surface, having slipped
# just enough to get there.  So the lever is where that surface sits, not
# whether the joint has been allowed to reach it.  This corrects the earlier
# reading that stage 4 was a delayed-onset problem.
#
# HOW MUCH THE ENVELOPE HAS TO COME DOWN.  Solve the level from Table 2 itself.
# At its own stage-4 slip the weakening consumed is (17/74.5)^1.10 = 0.1968, and
# the residual is 22.92*tan(6.50 deg) = 2.611 MPa, so the peak that delivers
# Table 2's tau = 9.38 MPa is (9.38 - 0.1968*2.611)/(1 - 0.1968) = 11.039 MPa,
# i.e. phi_peak = 25.717 deg at sigma'_n = 22.92 MPa.  The deck gives
# 22.72 + 5*log10(150/22.92) = 26.799 deg.  The envelope is 1.083 deg too high.
#
# WHY phi_r AND NOT JRC OR JCS.  Stage 3 must stay stuck (Table 2 d_s = 0.000
# there), so the cut has to lower the envelope at 22.92 MPa while holding it at
# 26.5 MPa.  Barton's curvature has the wrong sign for that: solving for the JRC
# that does it returns -16.8.  JCS is exactly equivalent to phi_r over a stress
# span this narrow (d phi / d log10 JCS = JRC, constant), and 150 MPa is the
# paper's own measured UCS.  phi_r is the only lever.
#
#   phi_r    tau_peak @ 22.92   margin @ Table 2 stage 3   margin @ model stage 3
#   22.72        11.577              +1.068 MPa                 +0.865 MPa
#   22.10        11.268              +0.712 MPa                 +0.508 MPa
#   21.60        11.020              +0.427 MPa                 +0.222 MPa
#
# D_c AND THE UNLOADING FLOOR ARE LEFT ALONE, DELIBERATELY.  Fitting Table 2's
# own tau / d_s pairs gives an implied D_c of 55-98 um, so the parent's 74.5 um
# was already inside the bracket, and the parent's unloading tau error decays to
# +0.05 MPa by stage 11 -- there is nothing there to fix.  105_04 (D_c 45 um)
# and 105_05 (weakening floor 3.15) both regressed, 6.139 -> 13.93 and 9.10.
#
# 21.60 deg is the full correction: it puts tau_peak at 11.020 MPa where the
# Table-2 inversion asks for 11.039.  It also thins the stage-3 margin to
# +0.222 MPa on the model's own state, so a few microns of slip may appear at
# stage 3 where Table 2 reports 0.000.  That is self-limiting -- slip there
# sheds frame load, which lowers sigma'_n toward Table 2's value, which the
# parent already overshoots by 0.11 MPa -- but it is why 106_09 runs the half
# step as insurance.  Everything else is unchanged.
# =============================================================================
# =============================================================================
...
bb_residual_friction_angle = 21.60 # 106_08: full Table-2 level correction, see header. Was 22.72.
                                     # --- history of the 22.72 value, kept verbatim; it does NOT describe the value above ---
...
# --- output ---
exodus_file_base = results_exodus_hpc_rorqual/106_08_sw4_phir21p60_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_08_sw4_phir21p60_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_08_sw4_phir21p60_ppfix_hpc
