...
# GENERATED 2026-08-15 from 68_02_sw4_bbfast_tail6p75_eta3p25_m0_shutin.i -- do not hand-edit; regenerate instead.
#
...
#      in 68_02_sw4_bbfast_tail6p75_eta3p25_m0_shutin.i.
#   3. Output file bases repointed to this deck's own name.
#
# The parent deck 68_02_sw4_bbfast_tail6p75_eta3p25_m0_shutin.i is left untouched as the reference configuration.
# ==============================================================================
# ===============================================================================
...
# --- output ---
exodus_file_base = results_exodus/68_02_sw4_bbfast_tail6p75_eta3p25_m0_shutin_kernel_SV
csv_file_base    = results_csv/68_02_sw4_bbfast_tail6p75_eta3p25_m0_shutin_kernel_SV
checkpoint_file_base = results_checkpoint/68_02_sw4_bbfast_tail6p75_eta3p25_m0_shutin_kernel_SV
...
[Functions]
  ...
  # DISCUSSION-SECTION DECK 2026-08-14: post-shut-in fault reactivation (ported from the
  # orca_3.0 v4 deck 10_SWS4_BBFast_mesh5_shutin.i, which this schedule was originally built
  # for -- see that deck's header for the full rationale). NOT the Ye & Ghassemi digitized
  # history: replaces the paper's rise-and-fall injection record with an idealized field-analog
  # schedule -- ramp to the same peak pressure/timing as the validated SW-S4 rise limb (28 MPa
  # reached ~1788 s), a short hold to let the slip event fully develop, then an ABRUPT shut-in
  # with the injection-node pressure relaxing exponentially toward the far-field/production
  # pressure -- a standard simplified proxy for wellbore pressure fall-off after shut-in. The
  # question this isolates: once the injection-point pressure has returned to near-ambient, does
  # slip/dilation along the fault keep growing (delayed, post-shut-in reactivation driven by
  # pressure already diffused into the fault) or does it arrest promptly? This is the mechanism
  # behind documented post-shut-in induced-seismicity cases (e.g. Pohang EGS, wastewater-disposal
  # sequences).
  #
  # Schedule: ambient 5 MPa (t<2s) -> linear ramp to 28 MPa over t=2..1788s (matches this deck's
  # own digitized rise, which independently also reaches ~28 MPa by ~1788s) -> hold at 28 MPa to
  # t=1988s (200s, well past onset) -> exponential shut-in relaxation toward 5 MPa with tau=150s
  # starting at t=1988s. end_time extended to 5000s for ~3000s of post-shut-in observation.
  [injection_pressure]
    type = ParsedFunction
    expression = 'if(t<2.0, 5.0e6, if(t<1788.0, 5.0e6+(28.0e6-5.0e6)*(t-2.0)/1786.0, if(t<1988.0, 28.0e6, 5.0e6+(28.0e6-5.0e6)*exp(-(t-1988.0)/150.0))))'
  []
...
[Executioner]
  ...
  line_search = l2
  start_time = 0
  end_time = 5000              # DISCUSSION: extended for ~3000s of post-shut-in observation window
