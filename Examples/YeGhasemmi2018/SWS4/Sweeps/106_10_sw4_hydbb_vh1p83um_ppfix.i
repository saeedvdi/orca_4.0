# =============================================================================
# 106_10 HYDRAULIC-CLOSURE PROBE -- SW-S4
# Parent: 93_07_sw4_final_theta30_jrc5_ppfix.i  (mean nRMSE 6.139 %)
#
...
#
# Unlike the two mode-I specimens SW-S4's stress term is live and roughly the
# right size -- sigma_0 = 1.05e-6 * 1.43e13 = 15.02 MPa against a 16-31 MPa
# operating range -- so this is a retune, not a repair.  What is wrong is the
# SHAPE: with p = 2.0 the term over-opens the joint through the loading stages
# and under-opens it at the event, leaving Q 15 % low at stage 6.
#
# FITTED CONSTANTS (sigma_0 = K_ni*V_m = 2.443e11 * 4.591e-5 = 11.2158 MPa held):
#   bb_stress_exponent        2.0      -> 3.28
#   bb_max_aperture_closure   1.05e-6  -> 1.8277e-6 m
#   bb_initial_normal_stiffness 1.43e13 -> 6.1366e12 Pa/m
#   aperture_scale                        UNCHANGED at 0.001 (SW-S4's a_h also
#     carries dilation and slip-damage terms, so it is not recoverable in closed
#     form here and is left out of the fit)
#
#   stage  sigma'_n  a_h Table2  a_h parent  a_h here   Q Table2   Q parent   Q here
#      4     24.49      0.830      0.829      0.818      0.035     0.0348    0.0334
#      6     16.05      1.070      1.009      1.057      0.113     0.0959    0.1104
#      8     19.39      0.850      0.876      0.884      0.037     0.0409    0.0441
#     11     24.86      0.740      0.761      0.749      0.005     0.0053    0.0051
#
# Predicted Q nRMSE 5.005 -> 2.183 %, mean 6.139 -> 5.57 %.  Independent of the
# strength arms: this deck does not touch the envelope.
#
# SW-S3 WAS CHECKED THE SAME WAY AND IS DELIBERATELY LEFT ALONE.  The same fit on
# 100_06 makes SW-S3's flow WORSE (3.060 -> 3.453 %), because Table 2's SW-S3
# pre-event apertures are flat to within their own scatter (1.22, 1.21, 1.20,
# 1.26, 1.25 um) and there is no closure signal in them to fit.  SW-S3's worst
# channels are tau (7.392) and d_n (6.174), neither of which is hydraulic.
# =============================================================================
# =============================================================================
...
# =============================================================================
# =============================================================================================
# 106_10_sw4_hydbb_vh1p83um_ppfix
#
...
bb_max_aperture_closure = 1.8277e-6   # 106_10: V_h fitted to Table 2 flow. Was 1.05e-6.
...
bb_initial_normal_stiffness = 6.1366e12 # 106_10: K_h = sigma_0/V_h with sigma_0 = K_ni*V_m = 11.2158 MPa.
bb_stress_exponent = 3.28             # 106_10: same exponent as this deck's MECHANICAL closure. Was 2.0.
...
# --- output ---
exodus_file_base = results_exodus_hpc_rorqual/106_10_sw4_hydbb_vh1p83um_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_10_sw4_hydbb_vh1p83um_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_10_sw4_hydbb_vh1p83um_ppfix_hpc
