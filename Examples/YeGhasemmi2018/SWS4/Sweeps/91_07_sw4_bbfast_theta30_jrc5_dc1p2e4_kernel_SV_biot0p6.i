# =============================================================================================
# 91_07_sw4_bbfast_theta30_jrc5_dc1p2e4_kernel_SV_biot0p6
#
# 91-SERIES: fix the RESIDUAL, keep the ONSET.  Back-analysis of the 90-series, 2026-08-17.
#
# Parent: 90_08_sw4_bbfast_theta30_jrc5_kernel_SV_biot0p6.i
#
...
#
# THIS DECK.  ONE change: bb_characteristic_slip_distance 7.45e-5 -> 1.20e-4 m (74.5 -> 120 um).
# Parent is 90_08 (JRC 5.0), NOT 90_07.
#
# WHY 90_08 AND NOT 90_07.  The published comparison preferred 90_07 on differential stress, but
# that verdict used the broken (sigma1 - 30e6) channel.  On the load-cell channel 90_08 ends at
# 5.36 MPa against 5.14 measured (+0.22) while 90_07 ends at 5.71 (+0.57), and 90_08 was already
# ahead on final shear traction (2.32 vs 2.25 measured, against 90_07's 2.47).  Corrected mean
# nRMSE: 90_08 6.8%, 90_07 6.9%.  Lower JRC wins on every residual channel, which is what the
# 90_07/90_08 pair was built to test, so the re-anchored phi_r = 22.72 deg is retained.
#
# WHAT IS ACTUALLY LEFT ON SW-S4.  Not the level -- onset, residual sigma_d, residual tau, final
# permeability and final dilation are all within a few percent.  What is wrong is the SHAPE of
# the transition.  SW-S4 is the one specimen whose measured response is staged: first weakening
# at 1025-1040 s, intermediate slip at 1320-1425 s, main transition at 1680-1695 s.  90_08 lands
# the main transition (1679 vs 1680 s on the 20 MPa stress-drop indicator) but arrives at every
# intermediate stage 59-90 s late and compresses them into one smoother event.  The measured
# joint takes ~650 s to go from first motion to full slip; the model takes far less.
#
# D_c IS THE CONTROL ON THAT DURATION.  SW-S4's total slip is 82.9 um against D_c = 74.5 um, so
# the joint crosses into the residual branch almost as soon as it moves and the transition
# collapses.  At D_c = 120 um the measured 80 um of slip reaches only 0.67 D_c, so weakening is
# still in progress when the experiment ends -- the event spreads out.
#
# PREDICTION.  Main transition holds at 1670-1700 s; the 0.020 mm and 0.040 mm slip stages move
# EARLIER, toward the measured 1320 and 1410 s; shear traction begins declining before 1200 s
# instead of 1352.  Final sigma_d rises above 5.36 MPa -- if it passes ~7 MPa, D_c has been
# raised too far and 91_08 brackets it from below.
#
# =============================================================================================
# ==============================================================================
...
bb_characteristic_slip_distance = 1.20e-4 # 67_01: in-bracket midpoint; trims stress with only ~0.3um predicted slip cost.  # 91_07: 74.5 -> 120 um, see header. Was 7.45e-5.
...
# --- output ---
exodus_file_base = results_exodus_hpc_rorqual/91_07_sw4_bbfast_theta30_jrc5_dc1p2e4_kernel_SV_biot0p6_hpc
csv_file_base    = results_csv_hpc_rorqual/91_07_sw4_bbfast_theta30_jrc5_dc1p2e4_kernel_SV_biot0p6_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/91_07_sw4_bbfast_theta30_jrc5_dc1p2e4_kernel_SV_biot0p6_hpc
