...
def figure_validation_histories_MC_BB_All_5(
    bb_cases: dict[str, str] = FINAL_BB_CASES,
    mc_cases: dict[str, str] = FINAL_MC_CASES
) -> plt.Figure:
    """Figure 3a: four mechanical channels for final BBFast and MC cases."""
    apply_agu_style()
    bb_scored = _score_map(bb_cases)
    mc_scored = _score_map(mc_cases)
    comparison_specs = tuple(spec for spec in VALIDATION_SPECS if spec[0] != "Q_ml_min")
    if len(comparison_specs) != 4:
        raise AssertionError(f"Expected four non-flow validation channels; got {len(comparison_specs)}")
    figure, axes = plt.subplots(4, 4, figsize=FIGURE_SIZES_IN["validation_histories"], squeeze=False)
    letters = iter(string.ascii_lowercase)
    for row_index, sample in enumerate(SAMPLE_ORDER):
        for column_index, (key, title) in enumerate(comparison_specs):
            ax = axes[row_index, column_index]
            bb_model, paper = _model_and_paper(bb_scored[sample], key)
            mc_model, _ = _model_and_paper(mc_scored[sample], key)
            ax.plot(STAGES, bb_model, color=BLUE, linewidth=1.2, zorder=3)
            ax.plot(STAGES, mc_model, color=ORANGE, linewidth=1.2, linestyle="--", zorder=2)
            ax.plot(
                STAGES,
                paper,
                linestyle="none",
    ...
    figure.legend(
        handles=(
            ...
            Line2D([], [], color=BLUE, linewidth=1.25, label="Barton-Bandis Interface Law simulation"),
            Line2D([], [], color=ORANGE, linewidth=1.25, linestyle="--", label="Mohr-Coulomb Interface Law simulation"),
            Patch(facecolor="#C3E720", edgecolor="none", label="Unloading stages"),
        ),
        loc="upper center",
...
OUTPUT_FILENAME = 'Figure_Validation_Histories_BB_MC.pdf'


def main() -> Path:
    """Build and export this figure; return the absolute PDF path."""
    figure = figure_validation_histories_MC_BB_All_5(FINAL_BB_CASES, FINAL_MC_CASES)
    validate_figure('validation_histories', figure)
