"""Batch validation-image export for the Ye and Ghassemi (2018) 106 sweep.

The public entry point, :func:`export_106_sweep_images`, discovers only 106-series
...
comparison in ``106_sweep/<sample>``.

Run directly from any directory with::

    python Examples/YeGhasemmi2018/all_fracture_image_export_106.py
"""
...
# Series 106 contains cases for these three specimens only. SWT2 has no 106 deck
...
def discover_hpc_results(study_dir: Path) -> pd.DataFrame:
    """Inventory every downloaded 106-series HPC CSV and matching input."""
    rows = []
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-directory",
        type=Path,
        help="output directory (default: <study-dir>/106_sweep)",
    )
    ...
    manifest = export_106_sweep_images(
        study_dir,
    ...
    print(
        f"106 sweep summary: {exported} results exported, "
