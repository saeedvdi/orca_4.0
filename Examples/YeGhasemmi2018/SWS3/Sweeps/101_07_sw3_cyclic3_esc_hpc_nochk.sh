...
#SBATCH --job-name=101_07_sw3_cyclic3_esc_hpc
...
#SBATCH --mem=32G
#SBATCH --output=logs/101_07_sw3_cyclic3_esc_hpc_%j.out
#SBATCH --error=logs/101_07_sw3_cyclic3_esc_hpc_%j.err
...
# deck runs 14443 s at dtmax = 0.75 s, i.e. >= 19257 steps.  At the
# measured 5.3 s/step on 16 ranks and an assumed 1.6x from 16 -> 32 ranks, that
# is ~17.7 h, so --time=48:00:00.  The 97-series jobs died because they
...
srun --mpi=pmi2 -n 32 /home/saeedvdi/links/projects/def-biaoli66/saeedvdi/projects/orca_4.0/orca-opt -i 101_07_sw3_cyclic3_esc.i \
