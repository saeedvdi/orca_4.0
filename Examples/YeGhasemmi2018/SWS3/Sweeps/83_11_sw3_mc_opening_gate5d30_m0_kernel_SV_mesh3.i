# ==============================================================================
# 83_11_sw3_mc_opening_gate5d30_m0_kernel_SV_mesh3
# GENERATED 2026-08-15 from 83_11_sw3_mc_opening_gate5d30_m0_kernel_SV.i -- do not hand-edit; regenerate instead.
#
# Change applied on 2026-08-15:
#   mesh_file: sw3_mesh_size5.e -> sw3_mesh_size3.e
#     size 3 is the FINER mesh: 10,368 -> 100,048 elements (9.6x).
#
...
#
# The parent deck 83_11_sw3_mc_opening_gate5d30_m0_kernel_SV.i is left untouched.
# ==============================================================================
######################################################################################
# 83_11_sw3_mc_opening_gate5d30_m0
# SW3 Level 83 kinematic-decoupling campaign.
# Parent: SW3_82_RESIDUAL_CORRECTIONS/82_11_sw3_mc_compliance7p15e13_ret0p525_m0
# Parameters: output-only reversible-opening gate: onset=5um; distance=30um; exponent=2
# Hypothesis: 82_11 opens reversibly by about 4.7-7.1um before failure despite near-zero measured dilation; a plastic-slip gate removes that premature stress-driven component and is essentially saturated by the 76um final slip.
# Decision rule: Keep only if pre-event/transition dilation RMSE improves while stress, shear slip, peak/end dilation, permeability, and flow remain within their 83_10 tolerances.
# This is a bounded back-analysis test, not an open-ended sweep.
######################################################################################
######################################################################################
# 83_11_sw3_mc_opening_gate5d30_m0
# SW3 Level 82 residual-correction campaign.
# Parent: SW3_81_SOURCE_CORRECTIONS/81_12_sw3_mc_retention0p50_gate50_m0
# Parameters: output compliance=7.15e-13m/Pa; post-event retention=0.525
# Hypothesis: The Level 81 opening decomposition predicts these values from the measured peak and terminal openings.
# Decision rule: Keep only if peak and terminal dilation errors are both below 1um while stress, slip, flow, and permeability remain on 81_12.
# This is a bounded back-analysis test, not an open-ended sweep.
######################################################################################
######################################################################################
# 83_11_sw3_mc_opening_gate5d30_m0
# SW3 Level 81 focused source-correction confirmation.
# Parent: SW3_80_POSTPEAK_SOURCE/80_11_sw3_mc_revretain0p38_from78_12_m0
# Parameters: reversible-opening retention=0.50 activated at 50um
# Hypothesis: A small upper memory bracket determines whether 0.38 under-retains after the history reset.
# Decision rule: Reject if peak dilation moves away from validation or terminal opening is over-retained.
# This is a confirmation bracket, not an open-ended sweep.
######################################################################################
######################################################################################
# 83_11_sw3_mc_opening_gate5d30_m0
# SW3 Level 80 post-peak source validation.
# Parent: SW3_78_MECHANICS_FIRST_LOCAL/78_12_sw3_mc_rough1p125_eta7p5e11_from76_11_m0
# Parameters: reversible opening retention=0.38; strength, slip, direct dilation, and hydraulics unchanged
# Hypothesis: About 38% of the 6.8um reversible-opening loss accounts for the 2.6um excessive modeled rebound; the new state changes reporting only.
# Decision rule: Keep if peak opening is unchanged and rebound/terminal opening improve without any change in traction, slip, permeability, or flow.
...
# Parent: SW3_76_FINAL_CALIBRATION/76_11_sw3_mc_final_peak1p13_smooth0p096_eta7p5e11_rev5p5e13_m0
# Parameters: mu_rough=1.125; mu_smooth=0.096; Ld=35um; eta=7.5e11; all other parameters held at 76_11
# Hypothesis: Lower rough-friction onset bracket with no viscosity confounding.
# Decision rule: Reject if it reproduces the very early precursor of the 77_11 mu_rough=1.12 case.
...
######################################################################################
# 83_11_sw3_mc_opening_gate5d30_m0
# SW3 76-series final calibration; one direct case per constitutive law.
# Selected completed parent: SW3_75_CORRECTED_DILATION_SWEEPS/75_12_sw3_mc_peak1p14_ld35_dil55_38_eta5e11_m0
# Final parameters: mu_rough=1.13; mu_smooth=0.096; Ld=35um; eta=7.5e11; C_rev=5.5e-13m/Pa
# Back-analysis: 75_12 was the best MC result. Interpolated peak friction moves first yield toward the measured time, higher viscosity spreads the overly abrupt traverse, the residual-friction increment corrects the low tail, and the independently fitted output-only compliance restores recoverable normal dilation.
# Numerical policy: automatic residual scaling OFF (retained from completed parent); dtmax=0.75 s.
# Permeability remains the local cubic-law value k=a_h^2/12.
# Audit note: the old SW3 digitization was low by exactly one factor of three;
# the corrected validation file now uses Ye & Ghassemi Table 2 values.
######################################################################################
######################################################################################
# 83_11_sw3_mc_opening_gate5d30_m0
# 75-series corrected-dilation back-analysis; direct run, no selection gate.
# Clean parent: SW3_72_BACKANALYSIS_CORRECTION/72_11_sw3_mc_peak1p22_ld45_dil29_eta5e11_m0
# Sweep parameters: mu_rough=1.14;Ld_um=35;dilation_deg=55_to_38;eta=5e11
# Purpose: central MC correction inferred between the 72/73 and 74 branches.
...
######################################################################################
######################################################################################
# 72_11 BACK-ANALYSIS CORRECTION: advanced Mohr-Coulomb
# Relative to completed 71_11, rough friction 1.02 -> 1.22 restores the measured peak,
# Ld 52 -> 45 um reaches the residual near 73 um, residual dilation 22 -> 29 deg fixes
# under-opening, and eta 3e12 -> 5e11 accelerates the very slow event. Secondary
# weakening remains zero so the former negative-strength runaway cannot recur.
######################################################################################
######################################################################################
# 71_11: SW3 advanced-MC bounded-tail correction candidate (eta=3e12 Pa.s/m)
# Direct run: common controls are baked into this input; no Stage-1 selection file is required.
# The unbounded 4-MPa secondary drop in 70_11 drove strength to its zero floor and
# caused the t=3567.55 s micro-step stall; this candidate removes that failure mode.
#
# Common parent: 59_07 corrected SW3 Table-2 / constant-piston / reaction-force deck.
# Constitutive material: ADOrcaDecoupledDilationRoughnessContactTractionCompressionTensile
# Calibration status: SW3 v12 constitutive calibration on corrected common experiment.
#
...
# --- mesh / geometry ---
mesh_file = mesh/sw3_mesh_size3.e   # SW-S3 rough saw-cut (theta=29deg, D=50.53mm, L=124.4mm), pre-tagged scaffold
...
# UNUSED after PST swap: dilation_angle_residual_degrees = 29.0 # DECK21: raised 15->22 (tan15=0.27->tan22=0.40). Deck19 dilation/slip ratio was ~0.29 (residual-dominated); target 0.031/0.077=0.40. Lifts peak dilation -0.021->~-0.031 mm and (via sigma'n relief) nudges slip 73->~77 um.
...
# dilation = g_np + d_rev.
reversible_normal_compliance = 7.15e-13  # SW3_July18-58/59_01: 3.2e-13 -> 0 (linear term OFF; DMGREV below)
...
reversible_normal_reference_stress = 32.1e6 # Pa: initial preload sigma'_n where d_rev = 0.
...
# --- output ---
exodus_file_base = results_exodus/83_11_sw3_mc_opening_gate5d30_m0_kernel_SV_mesh3
csv_file_base    = results_csv/83_11_sw3_mc_opening_gate5d30_m0_kernel_SV_mesh3
checkpoint_file_base = results_checkpoint/83_11_sw3_mc_opening_gate5d30_m0_kernel_SV_mesh3
...
[Mesh]
  ...
  [source_in]
    ...
    coord = '-0.0232438 0 0.0202670716'   # SW-S3 mesh injection node (v4 selection)
  ...
  [source_out]
    type = ExtraNodesetGenerator
    input = source_in
    coord = '0.0232438 0 0.1041329284'   # SW-S3 mesh production node (v4 selection)
...
[Materials]
  ...
  []
  [czm_contact]
    # Case 70_11: SW3 v12 constitutive calibration on the common corrected 59_07 experiment.
    # The 3e13 Pa.s/m viscosity is intentionally retained and must be reported as a
    # law-specific calibration/regularization, not as common experimental physics.
    ...
    roughness_decay_distance = 3.5e-5

    friction_coefficient_rough = 1.125
    friction_coefficient_smooth = 0.096
    friction_roughness_exponent = 1.0

    # NOTE: set directly (not via 0.0) — MOOSE brace substitution of the top-level
    # negative value was resolving to 0 here, silently zeroing the Coulomb intercept.
    cohesion_rough = 0.0
    cohesion_smooth = 0.0
    cohesion_roughness_exponent = 2.0

    normal_traction_tolerance = 0.0
    tangential_traction_tolerance = ${tangential_traction_tolerance}

    use_dilatancy = true
    dilation_angle_peak_degrees = 55.0
    dilation_angle_residual_degrees = 38.0
    dilation_decay_distance = 1.0e-4
    dilation_decay_exponent = 1.0
    max_plastic_slip_increment = 0.0
    tangential_viscosity = 7.5e11
    use_rate_and_state = true          # DECK52_12 (referenced RSF, deck-34 form)
    ...
    dissipation_margin = 0.02                          # DECK52_07
    secondary_weakening_strength = 0.0        # bounded tail: never subtract below Coulomb strength
    secondary_weakening_onset_slip = 38e-6  # DECK52_07
    secondary_weakening_distance = 36e-6      # DECK52_07
    dilation_opens_joint = true   # V15: kinematic dilatant hardening (gap opens)
    ...
    reversible_normal_opening_retention_fraction = 0.525  # output-only post-peak memory
    reversible_normal_opening_retention_activation_slip = 50e-6
    reversible_normal_opening_activation_slip = 5e-6
    reversible_normal_opening_activation_distance = 30e-6
    reversible_normal_opening_activation_exponent = 2.0
  []
  [czm_global_traction]
...
[Postprocessors]
  ...
  [injection_pressure_pp]
    type = PointValue
    variable = pore_pressure
    point = '-0.0232438 0 0.0202670716'  # SW3-v9 PPFIX: actual SW-S3 injection node (was the SW-S4 coordinate = 4 mm downstream)
  []
  [inj_reaction_sum_pp]
    type = NodalSum
  ...
  [pp_outlet_pp]
    type = PointValue
    variable = pore_pressure
    point = '0.0232438 0 0.1041329284'  # SW3-v9 PPFIX: actual SW-S3 production node (was the SW-S4 coordinate)
  []
  [pp_drop_pp]
    type = ParsedPostprocessor
  ...
  [czm_normal_dilation_paper_mm_pp]
    type = ParsedPostprocessor
    pp_names = czm_dn_total_pp                 # DECK23: total = g_np + reversible d_rev (was czm_dn_pp)
  ...
  [maximum_reversible_normal_opening_pp]
    type = ADSideAverageMaterialProperty
    property = maximum_reversible_normal_opening
    boundary = fracture_interface
  []
...
[Executioner]
  ...
  [TimeStepper]
    ...
    dt = 0.25                 # Level 83 controlled initial step
