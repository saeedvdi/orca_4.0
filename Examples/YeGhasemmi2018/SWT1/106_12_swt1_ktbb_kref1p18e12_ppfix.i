# =============================================================================
# 106_12 SHEAR-STIFFNESS PROBE -- SW-T1   (REQUIRES the orca_v9 source change)
# Parent: 100_01_swt1_vm55um_ppfix.i  (mean nRMSE 2.689 %, best SW-T1 on disk)
#
# THE GAP.  Every one of the four specimens creeps in shear before its slip
# event and this model does not.  Table 2's own d_s column, stages 1-5:
#
#   SW-T1  0.000 0.000 0.001 0.002 0.008 mm     model  0.000 0.000 0.000 0.000 0.001
#   SW-T2  0.000 0.001 0.003 0.007 0.015 mm     model  0.000 0.000 0.000 0.000 0.003
#
# and the creep happens while the shear stress is FALLING slightly (SW-T2:
# 74.87 -> 73.40 MPa).  What is actually changing over those stages is the
# effective normal stress, 66.7 -> 57.9 MPa.
#
# WHY NO VALUE OF penalty_tangent FIXES IT.  With a constant tangential
# stiffness the elastic shear jump is tau/k_t, and tau is flat, so the modelled
# d_s is flat too -- at k_t = 1e13 the change across stages 1-5 is 0.02 um, and
# at any other constant k_t it is still proportional to a flat tau.  Softening
# the penalty moves the DATUM, which the stage-1-referenced scoring removes, and
# changes nothing about the shape.  This is a structural limitation, not a
# calibration miss.
#
# WHAT THE LITERATURE SAYS THE STIFFNESS SHOULD BE.  Barton-Bandis give the
# joint shear stiffness as k_s = (100/L)*sigma'_n*tan(phi_peak) -- proportional
# to sigma'_n, so a joint held at constant shear stress slides forward as
# sigma'_n falls.  For these 50 mm specimens at sigma'_n ~ 66 MPa and
# phi_peak ~ 46 deg that is ~1.4e12 Pa/m, roughly 7x SOFTER than the 1e13 this
# campaign has used, and the campaign's value is a numerical penalty that was
# never checked against it.
#
# WHAT IS NEW IN THE SOURCE.  ADOrcaBartonBandisContactTractionFastAD gains an
# opt-in stress-dependent tangential stiffness (default false = byte-identical
# to every previous deck):
...
# within a step and the return map's algebra is untouched.  penalty_tangent is
# then the stiffness AT sigma_ref.  bb_tangential_stiffness is exported for
# audit.
#
# CALIBRATION.  Solve k_ref from Table 2 directly.  With m = 1 and sigma_ref =
# 65.47 MPa the elastic shear jump is tau*sigma_ref/(k_ref*sigma'_n), so
#   d_s(stage 5) - d_s(stage 1)
#     = 65.47e6*(67.73/57.82 - 67.50/65.68)/k_ref = 9.4067e6/k_ref,
# and Table 2's 0.008 mm gives k_ref = 1.176e12 Pa/m.  Rounded to 1.18e12.
# That is within 20 % of the Barton-Bandis estimate above, from a completely
# independent measurement -- which is the main reason to believe this axis.
#
# WHAT TO WATCH.  The absolute shear jump at stage 1 becomes ~57 um.  It is a
# datum offset and the stage-1-referenced scoring removes it, but it is real
# compliance and it WILL change the slip event: SW-T1 fires on a 1.31 MPa margin
# (1.9 % of the limit) at stage 5.  The event firing at all is the first gate on
# this deck, before any channel is scored.  106_13 is the half-strength arm that
# exists in case it does not.  min_tangential_stiffness_fraction = 0.05 keeps
# the penalty from collapsing as sigma'_n drops through the burst.
# =============================================================================
# =============================================================================
# 100_01 REFINEMENT PROBE -- SW-T1
# Parent: 99_01_swt1_vm50um_ppfix.i
# One axis only: mechanical BB maximum closure 50.00 -> 55.00 um. The offset is
# recomputed at sigma_n=31 MPa (47.7493 -> 51.6707 um), preserving initial
# seating. Linear response from 45.91/50.00 um predicts mean nRMSE near 2.88%,
# but this extrapolation must be confirmed by the nonlinear run.
# =============================================================================
# =============================================================================
# 99_01 CONTROLLED CALIBRATION PROBE -- SW-T1
# Parent: 93_01_swt1_final_c26p9_resc9p19_ppfix.i
# One material axis only: mechanical BB maximum closure 45.91 -> 50.00 um.
# The closure offset is recomputed at sigma_n=31 MPa (44.3306 -> 47.7493 um)
# so the initial joint seating is preserved. All strength and hydraulic
# parameters are unchanged. Hypothesis: improve normal-displacement recovery.
# =============================================================================
# =============================================================================
...
penalty_tangent = 1.18e12   # 106_12: now the tangential stiffness AT sigma_ref, not a fixed penalty. Was 1e13.
...
exodus_file_base = results_exodus_hpc_rorqual/106_12_swt1_ktbb_kref1p18e12_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_12_swt1_ktbb_kref1p18e12_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_12_swt1_ktbb_kref1p18e12_ppfix_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    maximum_closure = 5.500e-5       # 100_01: 55.00 um continuation bracket
    normal_closure_stress_exponent = 3.28
    normal_closure_offset = 5.167067369997e-5 # 100_01: closure(31 MPa), preserves initial seating
    ...
    use_stress_dependent_tangential_stiffness = true   # 106_12: Barton-Bandis k_s ~ sigma'_n, see header.
    tangential_stiffness_reference_stress = 65.47e6   # Pa: Table 2 stage-1 sigma'_n for this specimen.
