...
# The 94-series shear law is preserved. Shared SWT1 closure and hydraulic-scale
# refinements are aligned with the final renamed BBFast deck for a fair rerun.
# The companion Slurm array varies only MC-specific shear parameters.
#
# 94-SERIES -- MOHR-COULOMB BASELINE.  SW-T1 mesh 5
#
# Historical construction: 94_01 was built from the 93-series BBFast sibling by
# replacing [czm_contact]. This renamed center retains that MC shear law and
# aligns the later shared BBFast refinements documented above. The comparison
# therefore isolates the shear law as closely as the two formulations allow;
# BBFast's explicit normal-unload retention has no MC counterpart.
#
...
# without being updated. THIS DECK RUNS the Mohr-Coulomb pair cohesion_rough = 37.03 MPa, cohesion_smooth = 9.19 MPa. The refit fixes the parameterisation; the offset from it is calibrated, not derived.
#
...
exodus_file_base = results_exodus/SWT1_OrcaMohrCoulombContactTraction
csv_file_base    = results_csv/SWT1_OrcaMohrCoulombContactTraction
checkpoint_file_base = results_checkpoint/SWT1_OrcaMohrCoulombContactTraction
...
[Materials]
  ...
  [czm_contact]
    ...
    # BBFast sibling 94_01_swt1_mc_final.i.
    #
    ...
    #     normal stress sigma'_n* = 56.94 MPa (Table 2, last stick stage of
    #     stages 1-5), where the BB peak friction angle is 36.20 deg and the
    #     BB peak strength is 68.55 MPa.
    #   * the (mu_rough, c_rough) written here are pre-divided by
    #     Rbar_0 = 1.0000 so that MC's strength AT ZERO SLIP equals the BB peak,
    ...
    #   max |MC - BB| over the stick stages 1-5: 0.090 MPa
    #   max |MC - BB| over the full sigma'_n range:  1.00 MPa
    ...
    #      line through the onset tangent.  They separate by up to 1.00 MPa at
    ...
    maximum_closure = 5.500e-5
    normal_closure_stress_exponent = 3.28
    normal_closure_offset = 5.167067369997e-5
    ...
    roughness_decay_distance = 1.5e-4

    # --- Coulomb envelope transferred from Barton-Bandis --------------------
    friction_coefficient_rough = 0.5536
    friction_coefficient_smooth = 0.5717
    friction_roughness_exponent = 1.0     # linear in Rbar, matching BB's linear-in-W form
    cohesion_rough = 3.7034e7
    cohesion_smooth = 9.190e6
