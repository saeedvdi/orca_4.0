# =============================================================================
# 106_02 HYDRAULIC-CLOSURE PROBE -- SW-T1
# Parent: 100_01_swt1_vm55um_ppfix.i  (mean nRMSE 2.689 %, best SW-T1 on disk)
#
# WHY THIS AXIS.  The hydraulic aperture assembled by
# ADOrcaRoughnessDamageFracturePermeability is
#     a_h = a_h0 + stress_aperture + aperture_scale*mechanical_aperture + ...
# and Table 2's flow is the cubic law Q ~ a_h^3 dP, so Q is a direct readout of
# a_h.  Comparing the parent's exported hydraulic_aperture_um_pp against Table 2
# stage by stage (scripts/retune_aperture_law.py) shows the aperture, not the
# flow solver, carries the whole Q residual.
#
# WHAT IS NEW.  The stress term is refitted with its shape TIED TO THE
# MECHANICAL CLOSURE ALREADY IN THIS DECK: the same power-law exponent
# p = 3.28 and the same scale stress sigma_0 = K_ni*V_m, leaving only the
# amplitude V_h free.  That removes the degenerate (V_h, sigma_0) valley a free
# two-parameter fit falls into, and it says something physical -- the joint's
# hydraulic aperture closes on the same normalised law as its mechanical
# aperture, with its own amplitude.  aperture_scale is refitted jointly because
# the stress term now supplies part of the post-event reclosure shape the
# mechanical term was being stretched to cover alone.
#
# THE PARENT HAS NO STRESS-APERTURE TERM AT ALL.  normal_stress_aperture_compliance
# is 0.0 AND use_nonlinear_normal_closure is false, and the two branches of
# ADOrcaRoughnessDamageFracturePermeability::computeStressAperture are mutually
# exclusive, so normal_stress_aperture_pp is identically zero at every timestep.
# The parent's a_h is frozen at 1.630 um through stages 1-5 while Table 2 opens
# to 1.72 um.  This arm turns the term on for the first time on this specimen.
#
# FITTED CONSTANTS (sigma_0 = K_ni*V_m = 2.443e11 * 5.500e-5 = 13.4365 MPa held):
#   bb_stress_exponent        4.0      -> 3.28
#   bb_max_aperture_closure   1.2e-6   -> 9.8398e-6 m
#   bb_initial_normal_stiffness 1.25e13 -> 1.3655e12 Pa/m
#   aperture_scale            0.016    -> 0.01236
#
#   stage  sigma'_n  a_h Table2  a_h parent  a_h here   Q Table2   Q parent   Q here
#      1     65.68      1.630      1.630      1.630      0.053     0.0528    0.0528
#      5     57.82      1.720      1.630      1.657      0.389     0.3344    0.3513
#      6     32.53      4.050      4.171      4.052      6.220     6.7831    6.2169
#      9     37.27      3.490      3.675      3.490      1.900     2.2193    1.9008
#     11     40.91      3.360      3.543      3.302      0.462     0.5422    0.4390
#
# Predicted Q nRMSE 4.511 -> 0.306 %, mean 2.689 -> 1.85 %.  The prediction holds
# sigma'_n at the parent's values; aperture feeds back into pressure diffusion, so
# treat it as a ranking, not a forecast.  Everything else is unchanged.
# =============================================================================
# =============================================================================
...
aperture_scale = 0.01236  # 106_02: refitted jointly with the stress term. Was 0.016.
normal_stress_aperture_compliance = 0.0
reference_effective_normal_stress = 65470000

use_nonlinear_normal_closure = true   # 106_02: the parent had NO stress-aperture term at all.
nonlinear_closure_type = barton_bandis
bb_max_aperture_closure = 9.8398e-6   # 106_02: V_h fitted to Table 2 flow. Was 1.2e-6 (inactive).

bb_initial_normal_stiffness = 1.3655e12 # 106_02: K_h = sigma_0/V_h with sigma_0 = K_ni*V_m = 13.4365 MPa.
bb_stress_exponent = 3.28             # 106_02: same exponent as this deck's MECHANICAL closure. Was 4.0.
dilation_scale = 0.0
...
exodus_file_base = results_exodus_hpc_rorqual/106_02_swt1_hydbb_vh9p84um_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_02_swt1_hydbb_vh9p84um_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_02_swt1_hydbb_vh9p84um_ppfix_hpc
