...
#
# SW-T1's SPLIT, AND WHAT CAUSES IT.  91_02's residual cohesion is already at the stress/slip
# optimum (tau wants 8.48, sigma'n 8.47, d_s 9.05; 91_02 runs 9.19).  Only d_n and Q want more,
# and they want 12.5 -- far outside the bracket.  The defect is therefore NOT the strength.  It
# is the UNLOADING branch: over stages 6->11 the measurement recovers 44 um of normal closure
# (d_n -0.157 -> -0.113 mm as sigma'_n rises 31.79 -> 41.14 MPa), and 91_02 recovers only 20 um
# (-0.1586 -> -0.1387).  Since the hydraulic aperture follows d_n, the same shortfall is what
# leaves Q 40-70% high on every unloading stage.  SW-T2, whose d_n scores 2.06%, recovers
# 10.6 um against a measured 12 um on an almost identical sigma'_n swing -- so this is specific
# to SW-T1 and not a property of the law.
#
# THE PARAMETER.  normal_unload_retention_fraction subtracts that fraction of the RECOVERED
# closure from the closure fed to the BB normal law, so a high value forces the joint to close
# much further to reach a given sigma'_n on the unloading branch -- exactly the observed
# under-recovery.  SW-T1 carries 0.94, the highest of the four decks (SW-T2 0.84, SW-S3 0.06,
# SW-S4 0.04), and it has never been varied on this specimen.  The mechanical closure constants
# cannot be the cause: all four decks share SW-S4's fit (K_ni 2.443e11, V_m 45.91 um, p 3.28,
# c_0 44.33 um), and at that pre-seating the power law moves only 0.8 um across the whole
# unloading branch, so it is not the channel carrying the 26 um gap.
#
# THIS DECK: normal_unload_retention_fraction 0.94 -> 0.60.  Nothing else moves.
#
# PREDICTION, FALSIFIABLE.  d_n at stage 11 should move from -0.1387 toward the measured -0.113,
# and the unloading Q excess should shrink with it.  If d_n does NOT move, the retention branch
# is not the carrier and the 26 um is in the shear-dilation recovery instead -- which would be a
# model-form limit, not a parameter, and SW-T1 would be final at 91_02 as it stands.
#
...
exodus_file_base = results_exodus_hpc_rorqual/92_01_swt1_final_c26p9_resc9p19_unld0p60_hpc
csv_file_base    = results_csv_hpc_rorqual/92_01_swt1_final_c26p9_resc9p19_unld0p60_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/92_01_swt1_final_c26p9_resc9p19_unld0p60_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    normal_unload_retention_fraction = 0.60   # 92-series: was 0.94; see header.
