# =============================================================================
# 106_03 UNLOAD-RECLOSURE PROBE -- SW-T1
# Parent: 100_01_swt1_vm55um_ppfix.i  (mean nRMSE 2.689 %, best SW-T1 on disk)
#
# ONE LINE CHANGES: normal_unload_retention_fraction 0.94 -> 0.70.
#
# d_n is the parent's worst channel (4.584 %) and its error is entirely in the
# unloading branch: the model holds -0.125 mm of normal opening at stage 11
# where Table 2 has recovered to -0.113 mm.  updateNormalUnloadState retains
# normal_unload_retention_fraction of the recovered closure, so at 0.94 this
# joint keeps essentially all of it and never recloses.  The same over-open
# joint is what puts sigma'_n 0.15-0.23 MPa LOW and the flow 15-18 % HIGH over
# stages 9-11, so one parameter moves three channels the same way.
#
# PRECEDENT, STATED HONESTLY: SW-S3's best run (100_06) sits at 0.00 on this
# parameter -- it moved only 0.06 -> 0.00 to get there, so the STEP is not the
# evidence.  The LEVEL is: SW-S3 scores its best at essentially zero unload
# retention and shows none of the over-open unloading signature, while this
# specimen sits at 0.94 and shows all of it.  0.70 is a deliberately
# conservative first step on a specimen whose slip event lives on a 1.9 %
# strength margin -- see the 105_02/105_03 cliff.  Everything else is unchanged.
# =============================================================================
# =============================================================================
...
exodus_file_base = results_exodus_hpc_rorqual/106_03_swt1_unld0p70_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_03_swt1_unld0p70_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_03_swt1_unld0p70_ppfix_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    normal_unload_retention_fraction = 0.70   # 106_03: unload reclosure bracket, see header. Was 0.94.
