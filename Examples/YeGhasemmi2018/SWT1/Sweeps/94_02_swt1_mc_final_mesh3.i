# =============================================================================
# 94-SERIES -- MOHR-COULOMB BASELINE.  SW-T1 mesh 3
#
# Built from 93_02_swt1_final_c26p9_resc9p19_ppfix_mesh3.i by replacing ONE block: [czm_contact].
...
# =============================================================================
# =============================================================================
# 93-SERIES -- MESH AND POSTPROCESSOR AUDIT FIXES.  SW-T1 mesh 3
# Built from 92_07_swt1_final_c26p9_resc9p19_mesh3.i.  Constitutive parameters are UNCHANGED;
...
# =============================================================================
# =============================================================================================
# 94_02_swt1_mc_final_mesh3
#
# SW-T1 MESH CONVERGENCE at element size 3.  Built 2026-08-17, after the 92-series brackets.
#
# Parent: 91_02_swt1_bbfast_c26p9_resc9p19_kernel_SV_biot0p6.i  (the SW-T1 FINAL, mesh size 5)
#
# ONLY TWO THINGS CHANGE, and neither is a calibration parameter:
#
#     mesh_file   ye2018_sw_T1_mesh_size_5.e  ->  ye2018_sw_T1_mesh_size_3.e
#     source_in   '-0.018945000 0.0 0.034081662' -> '-0.018945000 0.0 0.034081662'
#     source_out  ' 0.019260000 0.0 0.095222443' -> ' 0.018945000 0.0 0.094718338'
#
# WHY THE COORDINATES MOVE.  ExtraNodesetGenerator with use_closest_node = true never errors.
# It searches the WHOLE mesh and runs BEFORE the fault split, so a coordinate that merely lies
# NEAR the fracture can snap to a BULK node and the run then injects into the matrix with no
# warning at all.  Source coordinates are therefore mesh-RESOLUTION specific, not just geometry
# specific.  SW-T2 was actually caught by this on its own size-3 mesh (its size-5 coordinate
# snapped to a bulk node 584.9 um away, beating the nearest interface node at 956.8 um).
#
# SW-T1 was NOT trapped -- on the size-3 mesh the nearest node to the mesh-5 coordinate is on
# the interface, at 594.4 um.  The coordinates above are pinned to that node exactly anyway, so
# the deck states the point it actually uses rather than one 594 um away.  Verified with
#     python3 scripts/check_source_nodes.py mesh/ye2018_sw_T1_mesh_size_3.e <x y z> <x y z>
#
# WHAT THIS RUN IS FOR.  SW-T1 is FINAL at 91_02 (mean nRMSE 4.34% over eleven Table-2 stages;
# Q 7.38, sigma'_n 1.98, tau 2.73, d_n 8.64, d_s 0.97).  The 92_01/92_02 bracket on
# normal_unload_retention_fraction FAILED -- see SWT1_FINAL.md section 4 -- so no calibration
# parameter is in play here.  This deck exists solely to show the reported response does not
# depend on discretisation.
#
# FALSIFIABLE PREDICTION.  Table-2 mean nRMSE within +-0.5 percentage points of 4.34%, and
# no single observable moving by more than 1.5 points.  flow_rate_validation_ml_min_pp is the
# mesh-independent flow channel and should be the steadiest; flow_rate_pp and
# flow_rate_mesh_geometry_ml_min_pp are NOT mesh-independent by construction (task #13) and are
# expected to shift -- that is a known property of those two channels, not a convergence failure.
# If the mean moves by more than 0.5 points, the mesh-5 result is not converged and the paper's
# SW-T1 numbers must be quoted from mesh 3 instead.
# =============================================================================================
# ==============================================================================
...
# fracture angle=32.0 deg, mesh=mesh/ye2018_sw_T1_mesh_size_3.e
...
mesh_file = mesh/ye2018_sw_T1_mesh_size_3.e
...
[Mesh]
  ...
  [source_in]
    ...
    coord = '-0.018945000 0.0 0.034081662'
  ...
  [source_out]
    type = ExtraNodesetGenerator
    input = source_in
    coord = '0.018945000 0.0 0.094718338'
...
[Materials]
  ...
  [czm_contact]
    ...
    # BBFast sibling 94_02_swt1_mc_final_mesh3.i.
    #
