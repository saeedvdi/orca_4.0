"""Batch validation-image export for the Ye and Ghassemi (2018) 105 sweep.

The public entry point, :func:`export_105_sweep_images`, discovers only 105-series
CSVs in the three participating per-sample ``results_csv_hpc*`` directories. For each result it
saves a 12-panel time-history comparison and an eight-panel Table 2 hold-stage
comparison in ``105_sweep/<sample>``.

Run directly from any directory with::

    python Examples/YeGhasemmi2018/all_fracture_image_export_105.py
"""
...
# Series 105 contains cases for these three specimens only. SWT2 has no 105 deck
# and is deliberately excluded from both discovery and output.
...
def discover_hpc_results(study_dir: Path) -> pd.DataFrame:
    """Inventory every downloaded 105-series HPC CSV and matching input."""
    rows = []
    for sample in SAMPLES:
        sample_dir = study_dir / sample
        input_by_stem = {
            path.stem: path for path in sample_dir.glob(f"{SERIES_GLOB}.i")
        }
        result_paths = sorted(
            {
                path.resolve()
                ...
                if root.is_dir()
                for path in root.glob(f"{SERIES_GLOB}.csv")
                if path.is_file()
            },
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="YeGhasemmi2018 directory (default: directory containing this script)",
    )
    parser.add_argument(
        "--output-directory",
        type=Path,
        help="output directory (default: <study-dir>/105_sweep)",
    )
    parser.add_argument("--dpi", type=int, default=160)
    parser.add_argument(
        "--keep-existing",
        action="store_true",
        help="do not overwrite PNGs that already exist",
    )
    ...
    ranking_path = (
        repository / "doc" / "independent_analysis"
        / "TABLE2_ERROR_ACCURACY_RANKING.csv"
    )
    ranking = pd.read_csv(ranking_path) if ranking_path.is_file() else None
    manifest = export_105_sweep_images(
        study_dir,
        table2_ranking=ranking,
        output_directory=args.output_directory,
        dpi=args.dpi,
        overwrite=not args.keep_existing,
    )
    exported = int(manifest["export_status"].eq("OK").sum())
    missing = int(manifest["result_file"].eq("").sum())
    errors = int(manifest["export_status"].str.startswith("ERROR").sum())
    print(
        f"105 sweep summary: {exported} results exported, "
        f"{missing} results missing, {errors} errors"
    )
