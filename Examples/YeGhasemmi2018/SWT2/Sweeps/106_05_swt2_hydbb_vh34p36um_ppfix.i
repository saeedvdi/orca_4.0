# =============================================================================
# 106_05 HYDRAULIC-CLOSURE PROBE -- SW-T2
# Parent: 100_04_swt2_apscale0p0177_ppfix.i  (mean nRMSE 2.132 %, best SW-T2 on disk)
#
...
#
# THE PARENT'S STRESS TERM IS SATURATED.  use_nonlinear_normal_closure is already
# true here, but sigma_0 = bb_max_aperture_closure * bb_initial_normal_stiffness
# = 1.2e-6 * 1.25e13 = 15.0 MPa against a 58-67 MPa pre-event operating range, so
# g(N) = N^p/(sigma_0^p + N^p) is pinned at ~0.999 and the whole term delivers
# 0.0023 um of opening where Table 2 opens by 0.58 um.  Stage-5 flow is 0.730
# against Table 2's 1.505 ml/min, -51.5 %.  These constants were never fitted on
# this specimen: they are SW-T1's, and SW-T1 was never bracketed on them either.
#
# FITTED CONSTANTS (sigma_0 = K_ni*V_m = 2.443e11 * 4.591e-5 = 11.2158 MPa held):
#   bb_stress_exponent        4.0      -> 3.28
#   bb_max_aperture_closure   1.2e-6   -> 3.43624e-5 m
#   bb_initial_normal_stiffness 1.25e13 -> 3.2640e11 Pa/m
#   aperture_scale            0.0177   -> 0.01093
#
#   stage  sigma'_n  a_h Table2  a_h parent  a_h here   Q Table2   Q parent   Q here
#      5     58.23      2.690      2.115      2.167      1.505     0.7296    0.7848
#      6     30.20      4.920      4.778      4.888     11.100    10.1795   10.9001
#      8     33.78      4.390      4.554      4.400      5.150     5.7476    5.1864
#     11     39.44      4.210      4.414      3.975      0.910     1.0468    0.7644
#
# Predicted Q nRMSE 4.336 -> 2.502 %, mean 2.132 -> 1.77 %.
#
# WHAT THIS ARM CANNOT FIX, AND WHY THAT MATTERS.  Table 2 takes a_h from 2.11 to
# 2.69 um (+27 %) over stages 1-5 while sigma'_n falls only 66.7 -> 57.9 MPa, and
# almost all of that jump is in the last two stages.  No smooth function of
# sigma'_n reproduces a kink like that, and the reason is visible in Table 2's own
# displacement column: SW-T2 has already crept 0.015 mm in shear by stage 5, where
# this model has 0.003 mm.  The residual at stage 5 after this fit (-0.72 ml/min)
# is PRE-PEAK CREEP, not closure, and it is the target of the separate
# stress-dependent shear-stiffness work -- not of this deck.
# =============================================================================
# =============================================================================
...
exodus_file_base = results_exodus_hpc_rorqual/106_05_swt2_hydbb_vh34p36um_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_05_swt2_hydbb_vh34p36um_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_05_swt2_hydbb_vh34p36um_ppfix_hpc
