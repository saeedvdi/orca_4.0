# =============================================================================================
# 91_03_swt2_bbfast_theta30_resc8p74_kernel_SV_biot0p6
#
...
#
# THIS DECK.  ONE change: residual_cohesion 1.0695e7 -> 8.74e6 Pa (-1.96 MPa).
#
# THE PEAK IS FINISHED.  90_03 is the best case in the whole campaign: 6.3% mean normalized
# RMSE with the corrected sigma_d channel, and all four event indicators within 5-28 s of the
# measurement (slip 2220 vs 2225, stress drop 2228 vs 2255).  cohesion stays at 3.320e7 Pa.
# 90_04 (3.500e7) locked completely, so the peak is bracketed and 33.20 MPa is inside it --
# do not move it.
#
# THE RESIDUAL.  90_03 ends at 67.36 MPa differential stress against 62.84 measured, +4.52 MPa.
# In the 30-degree paper frame that is 0.433012701892219*4.52 = 1.96 MPa of excess shear
# traction.  Final slip is 0.5514 vs 0.574 mm (4% low), so cutting the residual moves that the
# right way too.
#
# PREDICTION.  Onset unchanged (2220 +/- 10 s) -- residual_cohesion cannot touch the peak
# crossing.  Final sigma_d 62-64 MPa, final slip 0.565-0.580 mm, final shear traction 27.0-27.5
# against 27.21 measured.  If the onset moves at all, residual_cohesion is leaking into the peak
# envelope and the material's cohesion interpolation W = exp[-(s/D_c)^m] needs auditing.
#
# =============================================================================================
# ==============================================================================
...
#    without being updated. THIS DECK RUNS c = 33.20 MPa, c_res = 8.74 MPa. The refit fixes the parameterisation; the offset from it is calibrated, not derived.
#
...
exodus_file_base = results_exodus_hpc_rorqual/91_03_swt2_bbfast_theta30_resc8p74_kernel_SV_biot0p6_hpc
csv_file_base    = results_csv_hpc_rorqual/91_03_swt2_bbfast_theta30_resc8p74_kernel_SV_biot0p6_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/91_03_swt2_bbfast_theta30_resc8p74_kernel_SV_biot0p6_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    residual_cohesion = 8.74e6 # interlock surviving the burst; pins the post-burst stage. Table 2 shows this joint retaining most of its dilation, so it does not lose all interlock in one event.  # 91_03: -1.96 MPa, see header. Was 1.0695e7.
    use_dilatancy = true
