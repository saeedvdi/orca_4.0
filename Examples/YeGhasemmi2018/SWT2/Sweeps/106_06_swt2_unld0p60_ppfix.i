# =============================================================================
# 106_06 UNLOAD-RECLOSURE PROBE -- SW-T2
# Parent: 100_04_swt2_apscale0p0177_ppfix.i  (mean nRMSE 2.132 %, best SW-T2 on disk)
#
# ONE LINE CHANGES: normal_unload_retention_fraction 0.84 -> 0.60.
#
# SW-T2 carries SW-T1's unloading signature: a_h is +0.10 to +0.21 um too open at
# every unloading stage (7-11) and the flow is 7-16 % high, with sigma'_n and tau
# both drifting high by 0.30-0.70 and 0.52-1.21 MPa over the same stages.  All of
# that is one over-retained joint: updateNormalUnloadState keeps
# normal_unload_retention_fraction of the recovered closure, and 0.84 keeps most
# of it.  Unlike SW-T1 the peak stage (6) is 0.14 um LOW here, so this is
# deliberately the retention axis and not an aperture_scale trim -- a uniform
# scale cut would take the peak further down while fixing the tail.
#
# PRECEDENT, STATED HONESTLY: SW-S3's best run (100_06) sits at 0.00 on this
# parameter, though it moved only 0.06 -> 0.00 to get there -- the LEVEL is the
# evidence, not the step.  SW-S3 shows none of the over-open unloading signature
# at zero retention; this specimen sits at 0.84 and shows all of it.  0.60 keeps a margin against SW-T2's slip event, which fires on a
# 1.3 % strength margin at stage 5 (tau 74.01 against limit 74.98 MPa) -- thinner
# even than SW-T1's 1.9 %.  Everything else is unchanged.
# =============================================================================
# =============================================================================
...
exodus_file_base = results_exodus_hpc_rorqual/106_06_swt2_unld0p60_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_06_swt2_unld0p60_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_06_swt2_unld0p60_ppfix_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    normal_unload_retention_fraction = 0.60   # 106_06: unload reclosure bracket, see header. Was 0.84.
