...
#
# MODEL: ADOrcaPeakShelfTailFlowRSFContactTraction
######################################################################################
...
[Materials]
  ...
  []
  [czm]
    type = ADOrcaPeakShelfTailFlowRSFContactTraction
    boundary = ${frac_all}
    ...
    # peak == shelf == tail collapses the three-stage friction law to a constant mu.
    peak_friction_coefficient = ${fparse tan(friction_angle_deg * pi / 180.0)}
    shelf_friction_coefficient = ${fparse tan(friction_angle_deg * pi / 180.0)}
    tail_friction_coefficient = ${fparse tan(friction_angle_deg * pi / 180.0)}
    apparent_cohesion = 0.0
    dilation_work_fraction = 0.0
