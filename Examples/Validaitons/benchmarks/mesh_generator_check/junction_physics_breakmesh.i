######################################################################################
# GENERATOR EQUIVALENCE ON A JUNCTION: BreakMeshByBlockGenerator (the reference)
#
...
#
# This is `junction_physics_orca.i` with ONLY the [Mesh] block and the two fracture
# boundary names changed. It cuts the same fractures with MOOSE's stock generator, using
# the two-pass re-block that ../fracutre_interseciton_problem documents: block_pairs mode
# refuses to split a node touching more than two blocks, so the fractures have to be split
# one at a time with the flanks merged in between.
#
# Every postprocessor must agree with the Orca-generator deck.
######################################################################################

frac_v = block_left_block_right
frac_h = core_cap
