...
#
# MODEL: ADOrcaBartonBandisFlowRSFContactTraction
######################################################################################
...
[Materials]
  ...
  []
  [czm]
    type = ADOrcaBartonBandisFlowRSFContactTraction
    boundary = ${fracture}
    penalty_normal = 1e+12
    penalty_tangent = 1e+12
    initial_normal_stiffness = 1e+12
    use_hyperbolic_normal_closure = false
    contact_gap_regularization = 1e-12
    # jrc = 0 -> constant phi = 30 deg; rate-and-state and dilatancy disabled.
    jrc = 0.0
    jcs = 1.0e8
    ...
    use_dilatancy = false
    # jrc = 0 leaves the JRC mobilization scale undefined, so delta_p must be set
    # explicitly. Any value works here because with jrc = 0 the mobilized roughness angle
    # is identically zero; this only keeps the law's internal scale well defined.
    peak_shear_displacement = 1e-3
    mobilization_onset_slip = 0.0
    # NOTE: this law REQUIRES rsf_a > 0 -- it has no switch to disable rate-and-state, so
    # it cannot be reduced to a classical Coulomb interface exactly. rsf_a = 1e-9 makes the
    # direct effect a*p*asinh(z) ~ 1e-1 Pa against a ~1e7 Pa normal stress, i.e. 1e-8 of the
    # strength, which is far below the discretization error of this benchmark.
    rsf_a = 1e-9
    rsf_b = 0.0
  []
  [czm_global_traction]
