...
TABLE2_MECHANICAL_SPECS = (
    ("sigma_n_MPa", "Effective normal stress\n(MPa)", False),
    ("tau_MPa", "Shear stress\n(MPa)", False),
    ("dn_mm", "Normal dilation\n(mm)", False),
    ("ds_mm", "Shear displacement\n(mm)", False),
)

# The two test families occupy distinctly different mechanical ranges.  Pairing
# their y scales keeps comparisons within each family direct without flattening
# the lower-stress SW-S3/SW-S4 responses against the SW-T1/SW-T2 range.
TABLE2_SCALE_GROUPS = (
    ("SWT1", "SWT2"),
    ("SWS3", "SWS4"),
)
...
def figure_table2_mechanical(
    bb_cases: dict[str, str] = FINAL_BB_CASES,
    mc_cases: dict[str, str] = FINAL_MC_CASES,
) -> plt.Figure:
    """Figure 3b: four Table 2 mechanical quantities for all specimens."""
    return _figure_table2_specimens(TABLE2_MECHANICAL_SPECS, "table2_mechanical", bb_cases, mc_cases)


OUTPUT_FILENAME = 'Figure_Table2_Mechanical_All_Specimens.pdf'


def main() -> Path:
    """Build and export this figure; return the absolute PDF path."""
    figure = figure_table2_mechanical(FINAL_BB_CASES, FINAL_MC_CASES)
    validate_figure('table2_mechanical', figure)
