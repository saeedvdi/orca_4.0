...
#            IMPERMEABLE FAULT -- only the footwall compartment is pressurized
#
...
#
# THIS CASE: IMPERMEABLE FAULT. The fault seals, so only the down-thrown compartment
# on the far side of it is pressurized and the sampled compartment stays at its
# initial pressure.
# The signature of that in the reference data is exact: delta sigma_xx = -delta
# sigma_zz at every sampled point, which holds only where delta p = 0.
######################################################################################
...
reservoir_left_bottom = -200       # x < x_fault, down-thrown; pressurized here
...
[Functions]
  # IMPERMEABLE: only the down-thrown compartment, on the far side of the fault
  # from the sampled column, is pressurized.
  [delta_p_fn]
    type = ParsedFunction
...
[Postprocessors]
  ...
  [dp_volume_avg_expected]
    type = ConstantPostprocessor
    value = ${fparse 0.14783510 * pressure_buildup}
  []
  [dtotal_xx_min_MPa]
