...
#
# MODEL: ADOrcaDecoupledDilationRoughnessContactTractionCompressionTensile
######################################################################################
...
[Materials]
  ...
  [czm]
    ...
    boundary = ${frac_all}
    # Pre-existing frictional joint: start fully damaged, no tensile cohesion.
    enable_tensile_cohesion = false
    penalty_normal = 1e+12
    penalty_tangent = 1e+12
    use_hyperbolic_normal_closure = false
    # friction_rough == friction_smooth and cohesion_rough == cohesion_smooth make the
    # roughness interpolation inert, so mu is constant at tan(30 deg), matching the GEOS
    # <Coulomb> law with defaultFrictionCoefficient = 0.577350269.
    friction_coefficient_rough = ${fparse tan(friction_angle_deg * pi / 180.0)}
    friction_coefficient_smooth = ${fparse tan(friction_angle_deg * pi / 180.0)}
    cohesion_rough = 0
    cohesion_smooth = 0
    use_dilatancy = false
    use_rate_and_state = false
  []
  [czm_global_traction]
