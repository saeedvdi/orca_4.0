...
#SBATCH --job-name=sws4_r117_peak
...
#SBATCH --mem=24G
#SBATCH --array=0-5%3
#SBATCH --output=slurm_sws4_r117_peak_%A_%a.out
#SBATCH --error=slurm_sws4_r117_peak_%A_%a.err
...
screen_base=proposed_inputs/sws4_recalibration_wave1_20260902/peak_screen/${stem}
...
mkdir -p proposed_inputs/sws4_recalibration_wave1_20260902/{peak_screen,logs}
cp "${source_input}" "${installed_input}"

echo "Peak-stage screen: ${stem} (array task ${SLURM_ARRAY_TASK_ID})."
srun --mpi=pmi2 -n 8 "${ORCA}" -i "${installed_input}" \
  Executioner/end_time=1810 \
  Outputs/exodus/enable=false \
  Outputs/chk/enable=false \
  "csv_file_base=${screen_base}"
echo "Completed peak-stage screen: ${stem}."
