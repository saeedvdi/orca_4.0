...
#SBATCH --mem=16G
#SBATCH --array=0-7
#SBATCH --output=slurm_orca_remaining_%A_%a.out
#SBATCH --error=slurm_orca_remaining_%A_%a.err
...
# configured end time.  The locally interrupted cases restart cleanly on HPC.
cases=(
  ...
  "SWS4:115_04_sws4_extended_depressurization_ppfix"
)
...
srun --mpi=pmi2 -n 32 "${ORCA}" -i "proposed_inputs/${case_stem}.i" \
