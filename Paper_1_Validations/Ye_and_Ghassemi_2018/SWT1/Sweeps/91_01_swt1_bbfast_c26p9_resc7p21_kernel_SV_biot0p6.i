# =============================================================================================
# 91_01_swt1_bbfast_c26p9_resc7p21_kernel_SV_biot0p6
#
...
#
# THIS DECK.  Two changes, both derived, both on SW-T1.
#
# PEAK cohesion 2.639e7 -> 2.688e7 Pa (+0.49 MPa).  90_01 crosses zero at 1600 s, on the ramp
# from the 24 MPa hold to the 28 MPa plateau; the measured event is at 1650 (slip) to 1725 s
# (stress drop), i.e. on the plateau.  90_02 (c = 28.00 MPa) still holds +1.50% margin at
# 28 MPa and never fails.  The measured cohesion sensitivity is (4.17 - 2.02)/1.61 = 1.34
# percentage points of margin per MPa of cohesion, so m(28 MPa) = 0 at
# c = 28.00 - 1.50/1.34 = 26.88 MPa.  At that value the margin during the 24 MPa hold is
# 3.32 - 1.12*1.34 = +1.82%, so it cannot fail early.
#
# HONEST LIMIT ON THE ONSET.  90_02's margin decays only 0.09 pp over the whole 28 MPa plateau,
# so hitting 1700-1725 s rather than ~1650 s would need the margin at plateau start to land in
# a 0.09 pp window -- 67 kPa of cohesion.  Onset is NOT tunable below +/-70 s with a static
# envelope.  The 70 s the experiment takes between first slip (1650) and stress drop (1720) is
# a transition-DURATION the model covers in 10 s; that is a model-form gap, not a parameter.
#
# RESIDUAL cohesion 1.1176e7 -> 7.21e6 Pa (-3.97 MPa).  90_01 ends 8.83 MPa high in differential
# stress; in the paper frame that is 0.449397*8.83 = 3.97 MPa of excess shear traction, which is
# exactly a residual-cohesion excess.  It should also fix the slip, which is 0.4928 vs 0.542 mm
# measured (9% LOW) -- less residual strength means more slip, so both errors move the right way.
#
# PREDICTION.  Onset 1640-1660 s (slip indicator on the measured 1650); final sigma_d 63-65 MPa
# against 62.68; final slip 0.52-0.55 mm against 0.542.  If the onset lands before 1600 s the
# cohesion sensitivity is wrong; if the final sigma_d overshoots below 60 MPa, residual_cohesion
# was cut too far and 91_02 is the answer.
#
# =============================================================================================
# ==============================================================================
...
# without being updated. THIS DECK RUNS c = 26.88 MPa, c_res = 7.21 MPa. The refit fixes the parameterisation; the offset from it is calibrated, not derived.
#
...
exodus_file_base = results_exodus_hpc_rorqual/91_01_swt1_bbfast_c26p9_resc7p21_kernel_SV_biot0p6_hpc
csv_file_base    = results_csv_hpc_rorqual/91_01_swt1_bbfast_c26p9_resc7p21_kernel_SV_biot0p6_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/91_01_swt1_bbfast_c26p9_resc7p21_kernel_SV_biot0p6_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    cohesion = 2.688e7 # asperity interlock of a MATED Mode-I fracture; pins the peak envelope through Table 2's last stick stage.  # 91_01: +0.49 MPa, see header. Was 2.639e7.
    residual_cohesion = 7.21e6 # interlock surviving the burst; pins the post-burst stage. Table 2 shows this joint retaining most of its dilation, so it does not lose all interlock in one event.  # 91_01: -3.97 MPa, see header. Was 1.1176e7.
    use_dilatancy = true
