...
#
# Parent: 89_04_swt1_bbfast_cohesion_kernel_SV_biot0p6.i (SW-T1, cohesion refit)
#   scored 26.7% mean normalized RMSE, event 530-595 s EARLY, best final flow/permeability/dilation/sigma_n of the complete cases
#
# WHY.  Every scored case in this campaign fails at the injection step where its strength margin
# m = (tau_lim - tau)/tau_lim crosses zero.  Measured margins (min over each 1-MPa injection bin):
#
#     specimen  case              m @5 MPa   crosses 0 at   timing vs validation
#     SW-T1     sweep19 b0.6        +11.9%     26 MPa         +22..86 s   (best complete)
#     SW-T1     89_04 +cohesion      +4.4%     19-20 MPa      -530..595 s
#     SW-T2     sweep21 b0.6        +12.4%     25-26 MPa      +33..41 s   (best)
#     SW-T2     89_03                +11.5%    23-24 MPa      -355..360 s
#     SW-T2     89_05 +cohesion      +5.1%     21 MPa         -430..445 s
#     SW-S3     84_01 baseline      +23.8%     25-26 MPa      -29..+6 s   (best)
#     SW-S3     86_01 phir8.45      +28.7%     26-27 MPa      -3..+30 s
#     SW-S3     89_02 paperJRC      +28.1%     22-23 MPa      -360..390 s
#     SW-S4     89_06               +13.5%     17-18 MPa      +50..73 s   (best)
#     SW-S4     89_01 paperJRC      +13.1%     14-15 MPa      early
#
...
# which for SW-T1/T2 is ~290 s and for SW-S3 ~350 s.  That is the entire "several hundred seconds
# too early" signature; the 89-series is not mistimed, it is one step too weak.
#
# SW-T1/T2 SPECIFIC.  The cohesion refit traded phi_r 44.1deg -> 29.756deg for a constant
# cohesion.  tau_lim = c + sigma'_n tan(phi) is exact at only ONE sigma'_n, and the refit
# anchored it at the FAILURE stress.  At the much higher pre-injection sigma'_n the flat
# cohesion line sits ~7 percentage points below the steep friction line it replaced, which
# is exactly the drop from sweep19's +11.9% starting margin to 89_04's +4.4%.  Cohesion
# adds LEVEL but no SLOPE, so the fix is a level correction: raise the PEAK cohesion only.
#
# THIS DECK: cohesion 2.465e7 -> 2.639e7 Pa (+1.74 MPa).  Derived: tau_lim at the crossing state
# (sigma'_n ~= 51 MPa) is 24.65 + 51*tan(29.756 + 15.32*log10(150/51)) = 62.96 MPa; the
# measured local margin slope is 0.425 %/MPa of injection pressure, so 1 MPa of cohesion
# buys 3.74 MPa of delay and 6.5 MPa of delay costs 1.74 MPa of cohesion.
# residual_cohesion stays at 1.1176e7 -- 89_04's residuals are its strong suit.
#
# PREDICTION (falsifiable): crossing moves from 19-20 to 26 MPa, i.e. failure on the top
# step.  Residual state is untouched -- residual_cohesion / residual friction / roughness_state
# residual are all unchanged from the parent, and those are what the parent already got right.
# If the crossing lands short of 26 MPa the level correction was under-estimated; if the
# joint never fails, over-estimated.  Either outcome brackets the answer in one submission.
# =============================================================================================
# ==============================================================================
# 90_01_swt1_bbfast_cohesion_c26p4_kernel_SV_biot0p6
...
# without being updated. THIS DECK RUNS c = 26.39 MPa, c_res = 11.18 MPa. The refit fixes the parameterisation; the offset from it is calibrated, not derived.
#
...
exodus_file_base = results_exodus_hpc_rorqual/90_01_swt1_bbfast_cohesion_c26p4_kernel_SV_biot0p6_hpc
csv_file_base    = results_csv_hpc_rorqual/90_01_swt1_bbfast_cohesion_c26p4_kernel_SV_biot0p6_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/90_01_swt1_bbfast_cohesion_c26p4_kernel_SV_biot0p6_hpc
...
[Mesh]
  ...
  [source_in]
    ...
    coord = '-0.019260000 0.0 0.033577557'
  ...
  [source_out]
    type = ExtraNodesetGenerator
    input = source_in
    coord = '0.019260000 0.0 0.095222443'
...
[Materials]
  ...
  [czm_contact]
    ...
    cohesion = 2.639e7          # asperity interlock of a MATED Mode-I fracture; pins the peak envelope through Table 2's last stick stage.
    residual_cohesion = 1.1176e7   # interlock surviving the burst; pins the post-burst stage. Table 2 shows this joint retaining most of its dilation, so it does not lose all interlock in one event.
    use_dilatancy = true
