...
#
# Parent: 89_05_swt2_bbfast_theta30_cohesion_kernel_SV_biot0p6.i (SW-T2, cohesion refit)
#   scored 24.9% mean normalized RMSE, event 430-445 s EARLY, but the BEST final state of all four SW-T2 cases (slip within 0.0001 mm, flow and permeability near-exact)
#
...
#
# SW-T1/T2 SPECIFIC.  Same defect as SW-T1: phi_r 46.29deg -> 29.756deg traded for constant
# cohesion, anchored at the failure stress, so the starting margin fell from sweep21's
# +12.4% to +5.1%.  89_05 already has the best endpoints in the SW-T2 set; the ONLY thing
# wrong with it is that it gets there one step too early.  Fix the level, keep everything.
#
# THIS DECK: cohesion 3.165e7 -> 3.320e7 Pa (+1.55 MPa).  Derived: tau_lim at the crossing state
# (sigma'_n ~= 50 MPa) is 31.65 + 50*tan(29.756 + 14.63*log10(150/50)) = 68.94 MPa; local
# margin slope 0.45 %/MPa, so 1 MPa of cohesion buys 3.2 MPa of delay.
# residual_cohesion stays at 1.0695e7.
#
# PREDICTION (falsifiable): crossing moves from 21 to 26 MPa, i.e. failure on the top
...
#    without being updated. THIS DECK RUNS c = 33.20 MPa, c_res = 10.70 MPa. The refit fixes the parameterisation; the offset from it is calibrated, not derived.
#
...
exodus_file_base = results_exodus_hpc_rorqual/90_03_swt2_bbfast_theta30_cohesion_c33p2_kernel_SV_biot0p6_hpc
csv_file_base    = results_csv_hpc_rorqual/90_03_swt2_bbfast_theta30_cohesion_c33p2_kernel_SV_biot0p6_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/90_03_swt2_bbfast_theta30_cohesion_c33p2_kernel_SV_biot0p6_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    residual_cohesion = 1.0695e7   # interlock surviving the burst; pins the post-burst stage. Table 2 shows this joint retaining most of its dilation, so it does not lose all interlock in one event.
    use_dilatancy = true
