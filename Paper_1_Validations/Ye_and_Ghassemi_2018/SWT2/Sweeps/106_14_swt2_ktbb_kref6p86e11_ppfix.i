# =============================================================================
# 106_14 SHEAR-STIFFNESS PROBE -- SW-T2   (REQUIRES the orca_v9 source change)
# Parent: 100_04_swt2_apscale0p0177_ppfix.i  (mean nRMSE 2.132 %, best SW-T2)
#
...
# audit.
#
# SW-T2 IS THE SPECIMEN THIS AXIS WAS BUILT FOR.  Its Table-2 pre-event creep is
# the largest of the four (0.015 mm by stage 5, 5x SW-T1's), and it is the direct
# cause of the one residual the hydraulic refit in 106_05 cannot touch: Table 2's
# aperture jumps 2.31 -> 2.69 um between stages 4 and 5 while sigma'_n falls only
# 2.1 MPa, a kink no smooth function of stress reproduces.  Stage-5 flow is the
# parent's single worst point, 0.730 against 1.505 ml/min.
#
# CALIBRATION.  With m = 1 and sigma_ref = 66.74 MPa,
#   d_s(stage 5) - d_s(stage 1)
#     = 66.74e6*(74.01/58.23 - 73.86/66.14)/k_ref = 1.0296e7/k_ref,
# and Table 2's 0.015 mm gives k_ref = 6.86e11 Pa/m.  Shear datum ~109 um.
#
# WHAT TO WATCH.  SW-T2's margin at stage 5 is 0.97 MPa, 1.3 % of the limit --
# thinner than SW-T1's.  Creep sheds frame load, which lowers tau AND sigma'_n
# together, so the event may move.  Gate this deck on the event firing before
# scoring any channel.  106_15 is the stiffer fallback.
# =============================================================================
# =============================================================================
...
#    without being updated. THIS DECK RUNS c = 33.20 MPa, c_res = 9.71 MPa. The refit fixes the parameterisation; the offset from it is calibrated, not derived.
#
...
penalty_tangent = 6.86e11   # 106_14: now the tangential stiffness AT sigma_ref, not a fixed penalty. Was 1e13.
...
aperture_scale = 0.0177 # 100_04: upper refinement bracket around fitted optimum
...
exodus_file_base = results_exodus_hpc_rorqual/106_14_swt2_ktbb_kref6p86e11_ppfix_hpc
csv_file_base    = results_csv_hpc_rorqual/106_14_swt2_ktbb_kref6p86e11_ppfix_hpc
checkpoint_file_base = results_checkpoint_hpc_rorqual/106_14_swt2_ktbb_kref6p86e11_ppfix_hpc
...
[Materials]
  ...
  [czm_contact]
    ...
    use_stress_dependent_tangential_stiffness = true   # 106_14: Barton-Bandis k_s ~ sigma'_n, see header.
    tangential_stiffness_reference_stress = 66.74e6   # Pa: Table 2 stage-1 sigma'_n for this specimen.
