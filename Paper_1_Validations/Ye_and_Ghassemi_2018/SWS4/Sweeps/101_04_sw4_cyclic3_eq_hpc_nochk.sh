...
#SBATCH --job-name=101_04_sw4_cyclic3_eq_hpc
...
#SBATCH --mem=32G
#SBATCH --output=logs/101_04_sw4_cyclic3_eq_hpc_%j.out
#SBATCH --error=logs/101_04_sw4_cyclic3_eq_hpc_%j.err
...
# deck runs 11614 s at dtmax = 1.5 s, i.e. >= 7743 steps.  At the
# measured 5.3 s/step on 16 ranks and an assumed 1.6x from 16 -> 32 ranks, that
# is ~7.1 h, so --time=24:00:00.  The 97-series jobs died because they
...
srun --mpi=pmi2 -n 32 /home/saeedvdi/links/projects/def-biaoli66/saeedvdi/projects/orca_4.0/orca-opt -i 101_04_sw4_cyclic3_eq.i \
